 ...
 *
 * $Id: Grammar.java,v 1.1 2007-10-19 15:55:01 soto-r Exp $
 */
...
/**
 * A Grammar holds a set of rules (which are stored in a symbol table). Most of
 * the time a grammar needs a code generator and an LLkAnalyzer too.
 */
public abstract class Grammar {
     ...
     * manager is responsible for maintaining the set of valid tokens, and // is
     * conceptually shared between the lexer and parser. This may be either a //
     ...
     * integer value. If the associated value is not an integer or is not in the
     * table, then throw an exception of type NumberFormatException.
     * 
     * @param key
            ...
            try {
                ...
                if (maxk <= 0) {
                    antlrTool.error("option 'k' must be greater than 0 (was "
                            + value.getText() + ")", getFilename(), value
                            .getLine(), value.getColumn());
                    maxk = 1;
                }
            } catch (NumberFormatException e) {
                antlrTool.error("option 'k' must be an integer (was "
                        + value.getText() + ")", getFilename(),
            ...
            try {
                ...
                antlrTool
                        .error(
                                ...
                                getFilename(), value.getLine(), value
                                        .getColumn());
            }
            return true;
        }
