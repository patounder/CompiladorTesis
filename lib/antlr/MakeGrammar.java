 ...
 *
 * $Id: MakeGrammar.java,v 1.1 2007-10-19 15:54:56 soto-r Exp $
 */
...
public class MakeGrammar extends DefineGrammarSymbols {
    ...
    protected Stack blocks = new LList(); // track

    // subrules--Stack<BlockContext>
    ...
    public void beginExceptionSpec(Token label) {
        ...
        if (label != null) {
            label.setText(StringUtils.stripFront(StringUtils.stripBack(label
                    .getText(), " \n\r\t"), " \n\r\t"));
        }
    ...
    public void beginTree(Token tok) throws SemanticException {
        if (!(grammar instanceof TreeWalkerGrammar)) {
            tool.error("Trees only allowed in TreeParser", grammar
     ...
     * 
     * @param g
     *            The Grammar that is being processed
     * @param lexRules
     *            A vector of lexer rules that will be used to create an
     *            alternate block.
     * @param rname
     *            The name of the resulting rule.
     */
    public static RuleBlock createNextTokenRule(Grammar g, Vector lexRules,
            String rname) {
        ...
        for (int i = 0; i < lexRules.size(); i++) {
            ...
            if (!r.isDefined()) {
                g.antlrTool.error("Lexer rule " + r.id.substring(1)
                        + " is not defined");
            } else {
                if (r.access.equals("public")) {
                    Alternative alt = new Alternative(); // create alt we'll
                    // add to ref rule
                    ...
                    if (targetRuleAlts != null && targetRuleAlts.size() == 1) {
                        Alternative onlyAlt = (Alternative) targetRuleAlts
                                .elementAt(0);
                        if (onlyAlt.semPred != null) {
                            // ok, has sem pred, make this rule ref alt have a
                            // pred
                            alt.semPred = onlyAlt.semPred;
                            // REMOVE predicate from target rule??? NOPE,
                            // another
                    ...
                    RuleRefElement rr = new RuleRefElement(g, new CommonToken(
                            ANTLRTokenTypes.RULE_REF, r.getId()),
    ...
    private AlternativeBlock createOptionalRuleRef(String rule, Token start) {
        ...
        String mrule = CodeGenerator.encodeLexerRuleName(rule); // can only be a
        // lexer rule!
    ...
    public void defineRuleName(Token r, String access, boolean ruleAutoGen,
        ...
        if (r.type == ANTLRTokenTypes.TOKEN_REF) {
            if (!(grammar instanceof LexerGrammar)) {
                tool.error("Lexical rule " + r.getText()
                        + " defined outside of lexer", grammar.getFilename(), r
                        .getLine(), r.getColumn());
            ...
            if (grammar instanceof LexerGrammar) {
                tool.error("Lexical rule names must be upper case, '"
                        + r.getText() + "' is not", grammar.getFilename(), r
                        .getLine(), r.getColumn());
    ...
    public void endExceptionSpec() {
        ...
        if (currentExceptionSpec == null) {
            tool
                    .panic("exception processing internal error -- no active exception spec");
        }
        if (context().block instanceof RuleBlock) {
            // Named rule
            ((RuleBlock) context().block)
                    .addExceptionSpec(currentExceptionSpec);
        } else {
            ...
            if (context().currentAlt().exceptionSpec != null) {
                tool.error(
                        ...
                        grammar.getFilename(), context().block.getLine(),
    ...
    public void endSubRule() {
        ...
        if (block.not && !(block instanceof SynPredBlock)
            ...
            if (!analyzer.subruleCanBeInverted(block,
                ...
                tool.error(
                        ...
                        grammar.getFilename(), block.getLine(), block
                                .getColumn());
            }
        }
    ...
    public void endTree() {
        super.endTree();
        BlockContext ctx = (BlockContext) blocks.pop();
        addElementToCurrentAlt(ctx.block); // add new TreeElement to enclosing
        // alt.
    }
    ...
    private void labelElement(AlternativeElement el, Token label) {
        if (label != null) {
            ...
            for (int i = 0; i < ruleBlock.labeledElements.size(); i++) {
                AlternativeElement altEl = (AlternativeElement) ruleBlock.labeledElements
                        .elementAt(i);
                String l = altEl.getLabel();
                if (l != null && l.equals(label.getText())) {
                    tool.error("Label '" + label.getText()
                            + "' has already been defined", grammar
    ...
    public void oneOrMoreSubRule() {
        if (context().block.not) {
            tool.error("'~' cannot be applied to (...)* subrule", grammar
                    .getFilename(), context().block.getLine(), context().block
                    .getColumn());
        }
        ...
        BlockContext old = (BlockContext) blocks.pop(); // remove old scope; we
        // want new type of
        // subrule
        blocks.push(new BlockContext());
        context().block = b;
    ...
    public void optionalSubRule() {
        ...
        // altogether!
        // It already thinks that we have a simple subrule, just add option
        // block.
        beginAlt(false);
        endAlt();
    }
    ...
    public void refCharLiteral(Token lit, Token label, boolean inverted,
        ...
        if (!(grammar instanceof LexerGrammar)) {
            tool.error("Character literal only valid in lexer", grammar
                    .getFilename(), lit.getLine(), lit.getColumn());
            return;
        }
        super.refCharLiteral(lit, label, inverted, autoGenType, lastInRule);
        CharLiteralElement cl = new CharLiteralElement((LexerGrammar) grammar,
                lit, inverted, autoGenType);

        // Generate a warning for non-lowercase ASCII when case-insensitive
        if (!((LexerGrammar) grammar).caseSensitive
                && cl.getType() < 128
                && Character.toLowerCase((char) cl.getType()) != (char) cl
                        .getType()) {
            tool
                    .warning(
                            "Character literal must be lowercase when caseSensitive=false",
                            grammar.getFilename(), lit.getLine(), lit
                                    .getColumn());
        }
        ...
        // if ignore option is set, must add an optional call to the specified
        // rule.
    ...
    public void refCharRange(Token t1, Token t2, Token label, int autoGenType,
            boolean lastInRule) {
        if (!(grammar instanceof LexerGrammar)) {
            tool.error("Character range only valid in lexer", grammar
                    .getFilename(), t1.getLine(), t1.getColumn());
            return;
        }
        ...
        if (!((LexerGrammar) grammar).caseSensitive) {
            if (rangeMin < 128
                ...
                tool
                        .warning(
                                "Character literal must be lowercase when caseSensitive=false",
                                grammar.getFilename(), t1.getLine(), t1
                                        .getColumn());
            }
            if (rangeMax < 128
                ...
                tool
                        .warning(
                                "Character literal must be lowercase when caseSensitive=false",
                                grammar.getFilename(), t2.getLine(), t2
                                        .getColumn());
            }
        }

        super.refCharRange(t1, t2, label, autoGenType, lastInRule);
        CharRangeElement cr = new CharRangeElement((LexerGrammar) grammar, t1,
                t2, autoGenType);
    ...
    public void refTokensSpecElementOption(Token tok, Token option, Token value) {
         ...
         */
        TokenSymbol ts = (TokenSymbol) grammar.tokenManager.getTokenSymbol(tok
                .getText());
        if (ts == null) {
        ...
        if (option.getText().equals("AST")) {
            ...
            grammar.antlrTool.error("invalid tokens {...} element option:"
                    + option.getText(), grammar.getFilename(),
    ...
    public void refReturnAction(Token returnAction) {
        if (grammar instanceof LexerGrammar) {
            String name = CodeGenerator
                    .encodeLexerRuleName(((RuleBlock) context().block)
                            .getRuleName());
            ...
            if (rs.access.equals("public")) {
                tool.warning("public Lexical rules cannot specify return type",
                        grammar.getFilename(), returnAction.getLine(),
    ...
    public void refRule(Token idAssign, Token r, Token label, Token args,
        ...
        if (grammar instanceof LexerGrammar) {
            // if (!Character.isUpperCase(r.getText().charAt(0))) {
            if (r.type != ANTLRTokenTypes.TOKEN_REF) {
                tool.error("Parser rule " + r.getText()
                        + " referenced in lexer");
                return;
            }
            if (autoGenType == GrammarElement.AUTO_GEN_CARET) {
                tool.error("AST specification ^ not allowed in lexer", grammar
    ...
    public void refStringLiteral(Token lit, Token label, int autoGenType,
        ...
        if (grammar instanceof TreeWalkerGrammar
            ...
            tool.error("^ not allowed in here for tree-walker", grammar
                    .getFilename(), lit.getLine(), lit.getColumn());
        }
    ...
    public void refToken(Token idAssign, Token t, Token label, Token args,
        ...
        if (grammar instanceof LexerGrammar) {
            ...
            if (autoGenType == GrammarElement.AUTO_GEN_CARET) {
                tool.error("AST specification ^ not allowed in lexer", grammar
                        .getFilename(), t.getLine(), t.getColumn());
            }
            if (inverted) {
                tool.error("~TOKEN is not allowed in lexer", grammar
                        .getFilename(), t.getLine(), t.getColumn());
            }
            refRule(idAssign, t, label, args, autoGenType);

            // if ignore option is set, must add an optional call to the
            // specified token rule.
            String ignore = ruleBlock.getIgnoreRule();
            if (!lastInRule && ignore != null) {
            ...
            if (idAssign != null) {
                tool
                        .error(
                                ...
                                grammar.getFilename(), idAssign.getLine(),
                                idAssign.getColumn());
            }
            if (args != null) {
                tool
                        .error(
                                "Token reference arguments only allowed in lexer",
                                grammar.getFilename(), args.getLine(), args
                                        .getColumn());
            }
            super.refToken(idAssign, t, label, args, inverted, autoGenType,
                    lastInRule);
    ...
    public void refTokenRange(Token t1, Token t2, Token label, int autoGenType,
            boolean lastInRule) {
        if (grammar instanceof LexerGrammar) {
            tool.error("Token range not allowed in lexer", grammar
                    .getFilename(), t1.getLine(), t1.getColumn());
            return;
        }
    ...
    public void synPred() {
        if (context().block.not) {
            tool.error("'~' cannot be applied to syntactic predicate", grammar
                    .getFilename(), context().block.getLine(), context().block
                    .getColumn());
        }
        // create the right kind of object now that we know what that is
