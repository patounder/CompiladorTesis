 ...
 *
 * $Id: Tool.java,v 1.1 2007-10-19 15:55:00 soto-r Exp $
 */
...
public class Tool {
    ...
    protected void checkForInvalidArguments(String[] args,
        ...
        for (int a = 0; a < args.length; a++) {
            if (!cmdLineArgValid.member(a)) {
                warning("invalid command-line argument: " + args[a]
                        + "; ignored");
            }
        }
    }
        ...
        try {
            ...
            if (destination_file.exists()) {
                if (destination_file.isFile()) {
                    ...
                    /*
                     * System.out.print("File " + dest_name + " already exists.
                     * Overwrite? (Y/N): "); System.out.flush(); response =
                     * in.readLine(); if (!response.equals("Y") &&
                     * !response.equals("y")) throw new
                     * FileCopyException("FileCopy: copy cancelled.");
                     */
                } else {
                    throw new FileCopyException("FileCopy: destination "
     ...
     * 
     * @param args
     *            The command-line arguments passed to main(). This wrapper does
     *            the System.exit for use with command-line.
     */
    ...
    public int doEverything(String[] args) {
        ...
        try {
            ...
            // Create the right code generator according to the "language"
            // option
            ...
            String codeGenClassName = "antlr." + getLanguage(behavior)
                    + "CodeGenerator";
            try {
                codeGen = (CodeGenerator) Utils
                        .createInstanceOf(codeGenClassName);
     ...
     *            The message
     * @param file
     *            The file that has the error (or null)
     * @param line
     *            The grammar file line number on which the error occured (or
     *            -1)
     * @param column
     *            The grammar file column number on which the error occured (or
     *            -1)
     */
    ...
    /**
     * Determine the language used for this run of ANTLR This was made a method
     * so the subclass can override it
     */
    ...
    private static void help() {
        ...
        System.err
                .println("  -o outputDir       specify output directory where all output generated.");
        System.err
                .println("  -glib superGrammar specify location of supergrammar file.");
        System.err
                .println("  -debug             launch the ParseView debugger upon parser invocation.");
        System.err
                .println("  -html              generate a html file from your grammar.");
        System.err
                .println("  -docbook           generate a docbook sgml file from your grammar.");
        System.err
                .println("  -diagnostic        generate a textfile with diagnostics.");
        System.err
                .println("  -trace             have all rules call traceIn/traceOut.");
        System.err
                .println("  -traceLexer        have lexer rules call traceIn/traceOut.");
        System.err
                .println("  -traceParser       have parser rules call traceIn/traceOut.");
        System.err
                .println("  -traceTreeParser   have tree parser rules call traceIn/traceOut.");
    ...
    /**
     * This method is used by all code generators to create new output files. If
     * the outputDir set by -o is not present it will be created here.
     */
    public PrintWriter openOutputFile(String f) throws IOException {
        ...
        return new PrintWriter(new PreservingFileWriter(outputDir
     ...
     * default implementation currently exits (via
     ...
     * override this to throw an unchecked exception such as
     ...
     *             method executes <code>fatalError("panic");</code>.
     */
    public void panic() {
        fatalError("panic");
    }
    ...
    /**
     * <p>
     * Process the command-line arguments. Can only be called by Tool. A bitset
     * is collected of all correct arguments via setArgOk.
     * </p>
     * 
     * @param args
     ...
     *            The message
     * @param file
     *            The file that has the warning (or null)
     * @param line
     *            The grammar file line number on which the warning occured (or
     *            -1)
     * @param column
     *            The grammar file line number on which the warning occured (or
     *            -1)
    ...
    public void warning(String[] s, String file, int line, int column) {
        ...
        for (int i = 1; i < s.length; i++) {
            System.err.println(FileLineFormatter.getFormatter()
                    .getFormatString(file, line, column)
                    + "    " + s[i]);
        }
    }

    /**
     * Support C++ & C# namespaces (for now). C++: Add a nested namespace name
     * to the current namespace. C# : Specify an enclosing namespace for the
     * generated code. DAW: David Wagner -- C# support by kunle odutola
     */
