 ...
 *
 * $Id: Lookahead.java,v 1.1 2007-10-19 15:55:01 soto-r Exp $
 */
 ...
 * particular lookahead computation for a <b>single</b> lookahead depth. Final
 * lookahead information is a simple bit set, but intermediate stages need
 * computation cycle and FOLLOW information.
 * 
 * <p>
 * Concerning the <tt>cycle</tt> variable. If lookahead is computed for a
 * RuleEnd node, then computation is part of a FOLLOW cycle for this rule. If
 * lookahead is computed for a RuleBlock node, the computation is part of a
 * FIRST cycle to this rule.
 * 
 * <p>
 * Concerning the <tt>epsilonDepth</tt> variable. This is not the depth
 * relative to the rule reference that epsilon was encountered. That value is
 * 
 * <pre>
 ...
 * </pre>
 * 
 * Used for rule references. If we try to compute look(k, ruleref) and there are
 * fewer than k lookahead terminals before the end of the the rule, epsilon will
 * be returned (don't want to pass the end of the rule). We must track when the
 ...
 * </pre>
 * 
 * LOOK(5, ref-to(b)) is {<EPSILON>} with depth = 4, which indicates that at 2
 * (5-4+1) tokens ahead, end of rule was reached. Therefore, the token at
 * 4=5-(5-4) past rule ref b must be included in the set == F. The situation is
 * complicated by the fact that a computation may hit the end of a rule at many
 ...
 * </pre>
 * 
 * Here, LOOK(3,ref-to(b)) returns epsilon, but the depths are {1, 2}; i.e.,
 * 3-(3-1) and 3-(3-2). Those are the lookahead depths past the rule ref needed
 * for the local follow.
 * 
 * <p>
...
public class Lookahead implements Cloneable {
    ...
    /**
     * Does this lookahead depth include Epsilon token type? This is used to
     * avoid having a bit in the set for Epsilon as it conflicts with parsing
     * binary files.
     */
    ...
    /**
     * What is the intersection of two lookahead depths? Only the Epsilon "bit"
     * and bitset are considered.
     */
