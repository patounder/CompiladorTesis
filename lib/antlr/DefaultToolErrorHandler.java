 ...
 *
 * $Id: DefaultToolErrorHandler.java,v 1.1 2007-10-19 15:54:58 soto-r Exp $
 */
...
class DefaultToolErrorHandler implements ToolErrorHandler {
     ...
     * 
     * @param output
     *            The array that will contain the token/character set dump, one
     *            element per k (lookahead) value
     * @param outputStartIndex
     *            The index into <code>output</code> that the dump should
     *            start at.
     * @param lexicalAnalysis
     *            true for lexical rule
     * @param depth
     *            The depth of the ambiguity
     * @param sets
     *            An array of bitsets containing the ambiguities
     */
    private void dumpSets(String[] output, int outputStartIndex,
            Grammar grammar, boolean lexicalAnalysis, int depth,
            Lookahead[] sets) {
        ...
        for (int i = 1; i <= depth; i++) {
            ...
            if (lexicalAnalysis) {
                ...
                line.append(sets[i].fset.toString(",", grammar.tokenManager
                        .getVocabulary()));
            }
     ...
     *            An array of bitsets containing the ambiguities
     * @param altIdx1
     *            The zero-based index of the first ambiguous alternative
     * @param altIdx2
     *            The zero-based index of the second ambiguous alternative
     */
    public void warnAltAmbiguity(Grammar grammar, AlternativeBlock blk,
            boolean lexicalAnalysis, int depth, Lookahead[] sets, int altIdx1,
            int altIdx2) {
        ...
        antlrTool.warning(output, grammar.getFilename(), blk.getLine(), blk
                .getColumn());
    ...
    public void warnAltExitAmbiguity(Grammar grammar,
            BlockWithImpliedExitPath blk, boolean lexicalAnalysis, int depth,
            Lookahead[] sets, int altIdx) {
