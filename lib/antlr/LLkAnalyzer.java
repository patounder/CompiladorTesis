 ...
 *
 * $Id: LLkAnalyzer.java,v 1.1 2007-10-19 15:55:01 soto-r Exp $
 */
...
public class LLkAnalyzer implements LLkGrammarAnalyzer {
     ...
     * children) or '.' as an alt by itself.
     */
    protected boolean altUsesWildcardDefault(Alternative alt) {
        ...
        if (head instanceof WildcardElement
                && head.next instanceof BlockEndElement) {
            return true;
        }
        return false;
    }
    ...
    /**
     * Is this block of alternatives LL(k)? Fill in alternative cache for this
     * block.
     * 
    ...
    public boolean deterministic(AlternativeBlock blk) {
        ...
        if (nalts == 1) {
            ...
            blk.getAlternativeAt(0).lookaheadDepth = 1; // set lookahead to
            // LL(1)
        ...
        outer: for (int i = 0; i < nalts - 1; i++) {
            ...
            for (int j = i + 1; j < nalts; j++) {
                ...
                do {
                    ...
                    // compare LOOK(alt i) with LOOK(alt j). Is there an
                    // intersection?
                    ...
                    if (DEBUG_ANALYZER)
                        System.out.println("intersection at depth " + k
                                + " is " + r[k].toString());
                    if (!r[k].nil()) {
                        haveAmbiguity = true;
                        k++;
                ...
                if (haveAmbiguity) {
                    ...
                    if (ai.synPred != null) {
                        ...
                        // for sure.
                        // If the (...)=> conflicts with alt j, j is
                        // nondeterministic.
                        // This prevents alt j from being in any switch
                        // statements.
                     ...
                     * tree, then remove elements from alt i lookahead from alt
                     * j's lookahead. Don't do an ambiguity warning.
                     */
                    else if (altUsesWildcardDefault(aj)) {
                        // System.out.println("removing pred sets");
                        // removeCompetingPredictionSetsFromWildcard(aj.cache,
                        // aj.head, grammar.maxk);
                        wildcardAlt = aj;
                    }
                     ...
                     */
                            ...
                            && (ai.head instanceof BlockEndElement || aj.head instanceof BlockEndElement)) {
                        // System.out.println("ai.head pts to
                        // "+ai.head.getClass());
                        // System.out.println("aj.head pts to
                        // "+aj.head.getClass());
                    }

                    /*
                     * If they have the generateAmbigWarnings option off for the
                     * block then don't generate a warning.
                     */
                    else if (!blk.generateAmbigWarnings) {
                    }

                    /* If greedy=true and *one* empty alt shut off warning. */
                    else if (blk.greedySet
                            && blk.greedy
                            && ((ai.head instanceof BlockEndElement && !(aj.head instanceof BlockEndElement)) || (aj.head instanceof BlockEndElement && !(ai.head instanceof BlockEndElement)))) {
                        // System.out.println("greedy set to true; one alt
                        // empty");
                    }

                    /* We have no choice, but to report a nondetermism */
                    else {
                        tool.errorHandler.warnAltAmbiguity(grammar, blk, // the
                                // block
                    ...
                    // a lookahead depth, k, was found where i and j do not
                    // conflict
        ...
        /*
         * if ( wildcardAlt!=null ) {
         * removeCompetingPredictionSetsFromWildcard(wildcardAlt.cache,
         * wildcardAlt.head, grammar.maxk); }
         */
    ...
    public boolean deterministicImpliedPath(BlockWithImpliedExitPath blk) {
        ...
        for (int i = 0; i < nalts; i++) { // check follow against all alts
            ...
            if (alt.head instanceof BlockEndElement) {
                tool.warning(
                        "empty alternative makes no sense in (...)* or (...)+",
                        grammar.getFilename(), blk.getLine(), blk.getColumn());
            }
            ...
            do {
                ...
                if (DEBUG_ANALYZER)
                    System.out.println("intersection at depth " + k + " is "
                            + r[k]);
                if (!r[k].nil()) {
                    haveAmbiguity = true;
                    k++;
            ...
            if (haveAmbiguity) {
                ...
                /*
                 * If the user specified warnWhenFollowAmbig=false, then we can
                ...
                /*
                 * If greedy=false then shut off warning...will have to add "if
                 * FOLLOW break" block during code gen to compensate for removal
                 * of warning.
                 */
                ...
                // no choice but to generate a warning
                    ...
                    tool.errorHandler.warnAltExitAmbiguity(grammar, blk, // the
                            // block
                            lexicalAnalysis, // true if lexical
                            grammar.maxk, // depth of ambiguity
                            r, // set of linear ambiguities
    ...
    /**
     * Compute the lookahead set of whatever follows references to the rule
     * associated witht the FOLLOW block.
     */
    public Lookahead FOLLOW(int k, RuleEndElement end) {
        ...
        if (end.cache[k] != null) {
            if (DEBUG_ANALYZER) {
                System.out.println("cache entry FOLLOW(" + k + ") for " + rule
                        + ": "
            ...
            if (re.cache[k] == null) {
                ...
                if (DEBUG_ANALYZER) {
                    System.out
                            .println("combining FOLLOW("
                                    + k
                                    + ") for "
                                    + rule
                                    + ": from "
                                    + end.cache[k].toString(",", charFormatter,
                                            grammar)
                                    + " with FOLLOW for "
                                    + ((RuleBlock) re.block).getRuleName()
                                    + ": "
                                    + re.cache[k].toString(",", charFormatter,
                                            grammar));
                }
                ...
                if (DEBUG_ANALYZER) {
                    System.out.println("saving FOLLOW("
                            ...
                            + ": from "
                            + end.cache[k]
                                    .toString(",", charFormatter, grammar));
                }
        ...
        for (int i = 0; i < rs.numReferences(); i++) {
            ...
            if (DEBUG_ANALYZER)
                System.out.println("FIRST of next[" + rule + "] ptr is "
                        + q.toString());
            /*
             * If there is a cycle then if the cycle is to the rule for this end
             * block, you have a cycle to yourself. Remove the cycle
             * indication--the lookahead is complete.
             */
            ...
            p.combineWith(q);
            if (DEBUG_ANALYZER)
                System.out.println("combined FOLLOW[" + rule + "] is "
                        + p.toString());
        }

        end.lock[k] = false; // we're not doing FOLLOW anymore

        // if no rules follow this, it can be a start symbol or called by a
        // start sym.
        ...
        if (p.fset.nil() && p.cycle == null) {
            if (grammar instanceof TreeWalkerGrammar) {
                ...
                // has been hit
                // EOF would be misleading; any character can follow a token
                // rule not just EOF
                // as in a grammar (where a start symbol is followed by EOF).
                // There is no
                // sequence info in a lexer between tokens to indicate what is
                // the last token
                // to be seen.
        ...
        if (DEBUG_ANALYZER) {
            System.out.println("saving FOLLOW(" + k + ") for " + rule + ": "
                    + p.toString(",", charFormatter, grammar));
        }
    ...
    private Lookahead getAltLookahead(AlternativeBlock blk, int alt, int k) {
        ...
        // System.out.println("getAltLookahead("+k+","+e+"), cache size is
        // "+a.cache.length);
        if (a.cache[k] == null) {
            p = e.look(k);
            a.cache[k] = p;
    ...
    public Lookahead look(int k, AlternativeBlock blk) {
        ...
        if (k == 1 && blk.not && subruleCanBeInverted(blk, lexicalAnalysis)) {
            ...
            if (lexicalAnalysis) {
                BitSet b = (BitSet) ((LexerGrammar) grammar).charVocabulary
                        .clone();
                ...
                p.fset.notInPlace(Token.MIN_USER_TYPE, grammar.tokenManager
                        .maxTokenType());
            }
        }
        currentBlock = saveCurrentBlock;
    ...
    public Lookahead look(int k, BlockEndElement end) {
        ...
        /*
         * Tree blocks do not have any follow because they are children of what
         * surrounds them. For example, A #(B C) D results in a look() for the
         * TreeElement end of NULL_TREE_LOOKAHEAD, which indicates that nothing
         * can follow the last node of tree #(B C)
         */
        ...
        /*
         * Syntactic predicates such as ( (A)? )=> have no follow per se. We
         * cannot accurately say what would be matched following a syntactic
         * predicate (you MIGHT be ok if you said it was whatever followed the
         * alternative predicted by the predicate). Hence, (like end-of-token)
         * we return Epsilon to indicate "unknown lookahead."
         */
     ...
     * <p>
     * If the atom has the <tt>not</tt> flag on, then create the set
     * complement of the tokenType which is the set of all characters referenced
     * in the grammar with this char turned off. Also remove characters from the
     * set that are currently allocated for predicting previous alternatives.
     * This avoids ambiguity messages and is more properly what is meant. ( 'a' |
     * ~'a' ) implies that the ~'a' is the "else" clause.
     * <p>
     * NOTE: we do <b>NOT</b> include exit path in the exclusion set. E.g., (
     * 'a' | ~'a' )* 'b' should exit upon seeing a 'b' during the loop.
     */
    public Lookahead look(int k, CharLiteralElement atom) {
        ...
        if (lexicalAnalysis) {
            if (atom.not) {
                ...
                if (DEBUG_ANALYZER)
                    System.out
                            .println("charVocab after removal of prior alt lookahead "
                                    + b.toString());
    ...
    /**
     * The lookahead of a (...)+ block is the combined lookahead of all
     * alternatives and, if an empty path is found, the lookahead of what
     * follows the block.
     */
    ...
    /**
     * Combine the lookahead computed for each alternative. Lock the node so
     * that no other computation may come back on itself--infinite loop. This
     * also implies infinite left-recursion in the grammar (or an error in this
     * algorithm ;)).
     */
     ...
     * They set epsilon and the code generator gens code to check for any
     * character. The code generator must remove the tokens used to predict any
     * previous alts in the same block.
     * <p>
     ...
     * <p>
     * if the end block is not locked and the FOLLOW is wanted, the algorithm
     * must compute the lookahead of what follows references to this rule. If
     * end block is locked, FOLLOW will return an empty set with a cycle to the
     * rule associated with this end block.
     */
     ...
     * 
     * <p>
     * When computing ruleref lookahead, we don't want the FOLLOW computation
     * done if an empty path exists for the rule. The FOLLOW is too loose of a
     * set...we want only to include the "local" FOLLOW or what can follow this
     ...
     * "local" FOLLOW. This variable is orthogonal to the <tt>lock</tt>
     * variable that prevents infinite recursion. noFOLLOW does not care about
     * what k is.
     */
    public Lookahead look(int k, RuleRefElement rr) {
        ...
        if (rs == null || !rs.defined) {
            tool.error("no definition of rule " + rr.targetRule, grammar
                    .getFilename(), rr.getLine(), rr.getColumn());
            return new Lookahead();
        }
        ...
        if (p.cycle != null) {
            tool.error("infinite recursion to rule " + p.cycle + " from rule "
                    + rr.enclosingRuleName, grammar.getFilename(),
    ...
    public Lookahead look(int k, StringLiteralElement atom) {
        ...
        if (lexicalAnalysis) {
            ...
            if (k > atom.processedAtomText.length()) {
                ...
                // get char at lookahead depth k, from the processed literal
                // text
    ...
    /**
     * The lookahead of a (...)=> block is the lookahead of what follows the
     * block. By definition, the syntactic predicate block defies static
     * analysis (you want to try it out at run-time). The LOOK of (a)=>A B is A
     * for LL(1) ### is this even called?
     */
    ...
    public Lookahead look(int k, TreeElement t) {
        if (DEBUG_ANALYZER)
            System.out.println("look(" + k + "," + t.root + "["
                    + t.root.getType() + "])");
        if (k > 1) {
    ...
    /**
     * The (...)* element is the combined lookahead of the alternatives and what
     * can follow the loop.
     */
     ...
     * lookahead returns with epsilon, at least one epsilon path exists (one
     * that consumes no tokens). The noFOLLOW flag being set for this
     * endruleblk, indicates that the a rule ref invoked this rule.
     * 
    ...
    public Lookahead look(int k, String rule) {
        ...
        if (rb.lock[k]) {
            if (DEBUG_ANALYZER)
                System.out.println("infinite recursion to rule "
                        + rb.getRuleName());
            return new Lookahead(rule);
        }
        ...
        if (rb.cache[k] != null) {
            if (DEBUG_ANALYZER) {
                System.out.println("found depth " + k + " result in FIRST "
                        + rule + " cache: "
        ...
        if (DEBUG_ANALYZER) {
            System.out.println("saving depth " + k + " result in FIRST " + rule
                    + " cache: "
                    + rb.cache[k].toString(",", charFormatter, grammar));
        }
        return p;
    ...
    /**
     * Remove the prediction sets from preceding alternatives and follow set,
     * but *only* if this element is the first element of the alternative. The
     * class members currenBlock and currentBlock.analysisAlt must be set
     * correctly.
     * 
     * @param b
     *            The prediction bitset to be modified
    ...
    private void removeCompetingPredictionSets(BitSet b, AlternativeElement el) {
        ...
        GrammarElement head = currentBlock
    ...
    /**
     * Remove the prediction sets from preceding alternatives The class members
     * currenBlock must be set correctly. Remove prediction sets from 1..k.
     * 
     * @param look
     *            The prediction lookahead to be modified
     * @el The element of interest
     * @k How deep into lookahead to modify
     */
    private void removeCompetingPredictionSetsFromWildcard(Lookahead[] look,
            AlternativeElement el, int k) {
        for (int d = 1; d <= k; d++) {
            for (int i = 0; i < currentBlock.analysisAlt; i++) {
                AlternativeElement e = currentBlock.getAlternativeAt(i).head;
    ...
    public boolean subruleCanBeInverted(AlternativeBlock blk, boolean forLexer) {
        ...
        for (int i = 0; i < blk.alternatives.size(); i++) {
            ...
            if (alt.synPred != null || alt.semPred != null
                    || alt.exceptionSpec != null) {
                return false;
            }
            // ... and there must be one simple element
            AlternativeElement elt = alt.head;
            if (!(elt instanceof CharLiteralElement
                    || elt instanceof TokenRefElement
                    || elt instanceof CharRangeElement
                    || elt instanceof TokenRangeElement || (elt instanceof StringLiteralElement && !forLexer))
