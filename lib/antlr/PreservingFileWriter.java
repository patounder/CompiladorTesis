 ...
 *
 * $Id: PreservingFileWriter.java,v 1.1 2007-10-19 15:55:00 soto-r Exp $
...
public class PreservingFileWriter extends FileWriter {
    ...
    public PreservingFileWriter(String file) throws IOException {
        ...
        if (parentdirname != null) {
            ...
            if (!parentdir.exists())
                throw new IOException("destination directory of '" + file
                        + "' doesn't exist");
    ...
    /**
     * Close the file and see if the actual target is different if so the target
     * file is overwritten by the copy. If not we do nothing
     */
