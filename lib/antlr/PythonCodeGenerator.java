...
//
// $Id: PythonCodeGenerator.java,v 1.1 2007-10-19 15:55:00 soto-r Exp $
...
public class PythonCodeGenerator extends CodeGenerator {
    ...
    /**
     * Create a Java code-generator using the given Grammar. The caller must
    ...
    /**
     * Get the identifier portion of an argument-action. For Python the ID of an
     * action is assumed to be everything before the assignment, as Python does
     * not support a type.
     * 
     * @param s
     *            The action text
    ...
    /**
     * Get the type portion of an argument-action. Python does not have a type
     * declaration before an identifier, so we just return the empty string.
     * 
     * @param s
     *            The action text
     * @param line
    ...
    public void gen(ActionElement action) {
        if (action.isSemPred) {
            ...
            if (tInfo.assignToRoot) {
                ...
                println("if (" + tInfo.refRuleRoot + " != None) and ("
                        + tInfo.refRuleRoot + ".getFirstChild() != None):");
                tabs++;
    ...
    public void gen(LexerGrammar g) throws IOException {
        ...
        println("self.caseSensitiveLiterals = "
                + toString(g.caseSensitiveLiterals));
    ...
    public void gen(OneOrMoreBlock blk) {
        ...
        /***********************************************************************
         * no matter what previous block did, here we have to continue one the
         * 'while block' level. Reseting tabs .. *
         **********************************************************************/
        tabs = _tabs_;
        println(cnt + " += 1");
        tabs = _tabs_;
    ...
    public void gen(WildcardElement wc) {
        ...
        if (grammar instanceof TreeWalkerGrammar) {
            ...
            if (grammar instanceof LexerGrammar
                    && (!saveText || wc.getAutoGenType() == GrammarElement.AUTO_GEN_BANG)) {
                println("self.text.setLength(_saveIndex)"); // kill text atom
                // put in buffer
            }
        } else {
    ...
    public PythonBlockFinishingInfo genCommonBlock(AlternativeBlock blk,
        ...
        for (int altDepth = startDepth; altDepth >= 0; altDepth--) {
            for (int i = 0; i < blk.alternatives.size(); i++) {
                ...
                nIF++;
                tabs++;
                genAlt(alt, blk); // this should have generated something. If
                // not
    ...
    protected void genMatchUsingAtomText(GrammarAtom atom) {
        ...
        if (grammar instanceof LexerGrammar
                && (!saveText || atom.getAutoGenType() == GrammarElement.AUTO_GEN_BANG)) {
            println("self.text.setLength(_saveIndex)"); // kill text atom put in
            // buffer
        }
    }
    ...
    public void genRule(RuleSymbol s, boolean startSymbol, int ruleNum) {
        ...
        if (rblk.returnAction != null) {
            if (rblk.returnAction.indexOf('=') >= 0)
                ...
                // mx
                println(extractIdOfAction(rblk.returnAction, rblk.getLine(),
                        rblk.getColumn())
                        + " = None");
            }
        }
        ...
        if (grammar instanceof LexerGrammar) {
            ...
            println("_saveIndex = 0"); // used for element! (so we can kill
            // text matched for element)
        }
    ...
    /**
     * Create and set Integer token type objects that map to Java Class objects
     * (which AST node to create).
     */
        ...
        for (int i = 0; i < v.size(); i++) {
            ...
            if (s != null) {
                ...
                if (ts != null && ts.getASTNodeType() != null) {
                    ...
                    println("self.tokenTypeToASTClassMap[" + ts.getTokenType()
                            + "] = " + ts.getASTNodeType());
                }
            }
        }
    ...
    public String getASTCreateString(GrammarAtom atom, String astCtorArgs) {
        if (atom != null && atom.getASTNodeType() != null) {
            // they specified a type either on the reference or in tokens{}
            // section
    ...
    /**
     * Get a string for an expression to generate creating of an AST node. Parse
     * the first (possibly only) argument looking for the token type. If the
     * token type is a valid token symbol, ask for it's AST node type and add to
     * the end if only 2 arguments. The forms are #[T], #[T,"t"], and as of
     * 2.7.2 #[T,"t",ASTclassname].
     * 
     * @param str
     *            The arguments to the AST constructor
     */
    public String getASTCreateString(String astCtorArgs) {
        ...
        if (nCommas < 2) { // if 1 or 2 args
            ...
            if (ts != null) {
                ...
                if (astNodeType != null) {
                    return "self.astFactory.create(" + astCtorArgs + emptyText
                            + ", " + astNodeType + ")";
                }
    ...
    public String getRangeExpression(int k, int[] elems) {
        ...
        return "(" + lookaheadString(k) + " >= " + getValueString(begin, true)
                + " and " + lookaheadString(k) + " <= "
    ...
    /**
     * Lexically process $var and tree-specifiers in the action. This will
     * replace #id and #(...) with the appropriate function calls and/or
     * variables etc...
     */
        ...
        if (g instanceof TreeWalkerGrammar) {
            ...
            if (!g.hasOption("ASTLabelType")) {
                g.setOption("ASTLabelType", new Token(
                        ANTLRTokenTypes.STRING_LITERAL, "<4>AST"));
            }
            labeledElementInit = "None";
            commonExtraArgs = "_t";
    ...
    protected void printGrammarAction(Grammar grammar) {
        println("### user action >>>");
        printAction(processActionForSpecialSymbols(grammar.classMemberAction
