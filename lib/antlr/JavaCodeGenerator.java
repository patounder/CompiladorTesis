 ...
 *
 * $Id: JavaCodeGenerator.java,v 1.1 2007-10-19 15:54:59 soto-r Exp $
 */
...
public class JavaCodeGenerator extends CodeGenerator {
    ...
    /**
     * Constant to indicate that we should continue mapping based on the last
     * mapping seen by the SMAP generator (there may have been intermediate
     * mappings generated by nested elements)
     */
    ...
    public void println(String code, int sourceStartLine) {
        if (sourceStartLine > 0 || sourceStartLine == CONTINUE_LAST_MAPPING)
            getPrintWriterManager().startSingleSourceLineMapping(
                    sourceStartLine);
        super.println(code);
        if (sourceStartLine > 0 || sourceStartLine == CONTINUE_LAST_MAPPING)
            getPrintWriterManager().endMapping();
        ...
        try {
            ...
            if (action.isSemPred) {
                ...
                if (tInfo.refRuleRoot != null) {
                    ...
                    // assignments
                    // with no other refs like "#rule = foo();" still forces
                    // this code to be
    ...
    public void gen(LexerGrammar g) throws IOException {
        ...
        try {
            ...
            currentOutput = getPrintWriterManager().setupOutput(antlrTool,
                    grammar);
            ...
            if (tsuffix != null) {
                String suffix = StringUtils.stripFrontBack(tsuffix.getText(),
                        "\"", "\"");
                if (suffix != null) {
                    print(", " + suffix); // must be an interface name for
                    // Java
                }
            }
            println(" {");
            ...
            print(processActionForSpecialSymbols(grammar.classMemberAction
                    ...
                    currentRule, null), grammar.classMemberAction.getLine());
            ...
            // These need to be before the literals since ANTLRHashString
            // depends on
            ...
            // nextToken() is a synthetic lexer rule that is the implicit OR of
            // all
            ...
            genBitsets(bitsetsUsed, ((LexerGrammar) grammar).charVocabulary
                    .size());
        ...
        try {
            ...
            if (generateNonGreedyExitPath) {
                ...
                println("if ( " + cnt + ">=1 && " + predictExit + ") break "
                        + label + ";", CONTINUE_LAST_MAPPING);
            }

            JavaBlockFinishingInfo howToFinish = genCommonBlock(blk, false);
            genBlockFinish(howToFinish, "if ( " + cnt + ">=1 ) { break "
                    + label + "; } else {" + throwNoViable + "}", blk.getLine());
        ...
        try {
            ...
            if (!(rs instanceof RuleSymbol)) {
                ...
                antlrTool.error("'" + rr.targetRule
                        + "' does not name a grammar rule", grammar
            ...
            // This is not AST construction; it is just the input tree node
            // value.
            ...
            // if in lexer and ! on rule ref or alt or rule, save buffer index
            // to kill later
            if (grammar instanceof LexerGrammar
            ...
            if (syntacticPredLevel == 0) {
                boolean doNoGuessTest = (grammar.hasSyntacticPredicate && (grammar.buildAST
                        && rr.getLabel() != null || (genAST && rr
                        .getAutoGenType() == GrammarElement.AUTO_GEN_NONE)));
                if (doNoGuessTest) {
                    // println("if (inputState.guessing==0) {");
                    // tabs++;
                }
                ...
                if (genAST) {
                    ...
                    case GrammarElement.AUTO_GEN_NONE:
                        // println("theASTFactory.addASTChild(currentAST,
                        // returnAST);");
                ...
                // if a lexer and labeled, Token label defined at rule level,
                // just set it here
    ...
    public void gen(TokenRangeElement r) {
        ...
        // match
        println("matchRange(" + r.beginText + "," + r.endText + ");", r
                .getLine());
        ...
        try {
            ...
            if (t.root.getLabel() != null) {
                println(t.root.getLabel() + " = _t==ASTNULL ? null :("
                        + labeledElementASTType + ")_t;", t.root.getLine());
            }
            ...
            if (t.root.getAutoGenType() == GrammarElement.AUTO_GEN_CARET) {
                antlrTool
                        .warning(
                                ...
                                grammar.getFilename(), t.getLine(), t
                                        .getColumn());
                t.root.setAutoGenType(GrammarElement.AUTO_GEN_NONE);
            }
    ...
    public void gen(WildcardElement wc) {
        ...
        try {
            ...
            if (grammar instanceof TreeWalkerGrammar) {
                ...
                if (grammar instanceof LexerGrammar
                        && (!saveText || wc.getAutoGenType() == GrammarElement.AUTO_GEN_BANG)) {
                    println("text.setLength(_saveIndex);"); // kill text atom
                    // put in buffer
                }
            } else {
        ...
        if (genAST) {
            if (blk instanceof RuleBlock) {
                ...
                println(rblk.getRuleName() + "_AST = (" + labeledElementASTType
                        + ")currentAST.root;", CONTINUE_LAST_MAPPING);
                if (grammar.hasSyntacticPredicate) {
                    // --tabs;
                    // println("}");
                }
            ...
            if (n < BITSET_OPTIMIZE_INIT_THRESHOLD) {
                ...
                for (int i = 0; i < elems.length;) {
                    ...
                    if ((i + 1) == elems.length || elems[i] != elems[i + 1]) {
                        // last number or no run of numbers, just dump
                        // assignment
    ...
    private void genBlockFinish(JavaBlockFinishingInfo howToFinish,
            String noViableAction, int line) {
        int oldDefaultLine = defaultLine;
        try {
            defaultLine = line;
    ...
    protected void genBlockInitAction(AlternativeBlock blk) {
        ...
        if (blk.initAction != null) {
            printAction(processActionForSpecialSymbols(blk.initAction, blk
                    .getLine(), currentRule, null), blk.getLine());
        }
    }
    ...
    public JavaBlockFinishingInfo genCommonBlock(AlternativeBlock blk,
        ...
        try {
            ...
            int startDepth = (grammar instanceof LexerGrammar) ? grammar.maxk
                    : 0;
            for (int altDepth = startDepth; altDepth >= 0; altDepth--) {
                ...
                for (int i = 0; i < blk.alternatives.size(); i++) {
                    ...
                    // if we made a switch above, ignore what we already took
                    // care
                    ...
                    try {
                        ...
                        if (alt.cache[1].fset.degree() > caseSizeThreshold
                            ...
                            if (alt.semPred != null) {
                                ...
                                ActionTransInfo tInfo = new ActionTransInfo();
                                String actionStr = processActionForSpecialSymbols(
                                        alt.semPred, blk.line, currentRule,
                                        tInfo);
            ...
            if (createdLL1Switch) {
                ...
                // return new JavaBlockFinishingInfo(ps+"}",true,nIF>0); //
                // close up switch statement
    ...
    private void genElementAST(AlternativeElement el) {
        ...
        try {
            ...
            // handle case where you're not building trees, but are in tree
            // walker.
            ...
            if (grammar.buildAST && syntacticPredLevel == 0) {
                ...
                if (needASTDecl) {
                    ...
                    if (el instanceof GrammarAtom) {
                        ...
                        if (ga.getASTNodeType() != null) {
                            genASTDeclaration(el, astNameBase, ga
                                    .getASTNodeType());
                ...
                if (el.getLabel() != null) {
                    if (el instanceof GrammarAtom) {
                        println(astName
                                + " = "
                                + getASTCreateString((GrammarAtom) el,
                                        elementRef) + ";");
                    } else {
                        println(astName + " = "
        ...
        for (int i = 0; i < ex.handlers.size(); i++) {
            ...
            try {
                ...
                println("catch (" + handler.exceptionTypeAndName.getText()
                        + ") {", handler.exceptionTypeAndName.getLine());
                tabs++;
                if (grammar.hasSyntacticPredicate) {
                    println("if (inputState.guessing==0) {");
                    tabs++;
                ...
                ActionTransInfo tInfo = new ActionTransInfo();
                printAction(processActionForSpecialSymbols(handler.action
                        .getText(), handler.action.getLine(), currentRule,
                        tInfo));
    ...
    protected void genMatchUsingAtomText(GrammarAtom atom) {
        ...
        try {
            ...
            if (grammar instanceof LexerGrammar
                    && (!saveText || atom.getAutoGenType() == GrammarElement.AUTO_GEN_BANG)) {
                println("text.setLength(_saveIndex);"); // kill text atom put in
                // buffer
            }
        } finally {
            defaultLine = oldDefaultLine;
    ...
    public void genRule(RuleSymbol s, boolean startSymbol, int ruleNum) {
        ...
        try {
            ...
            if (grammar instanceof LexerGrammar) {
                ...
                // This is a horrible hack to support the built-in EOF lexer
                // rule.
                if (s.getId().equals("mEOF"))
                    println("_ttype = Token.EOF_TYPE;");
                else
                    println("_ttype = " + s.getId().substring(1) + ";");
                println("int _saveIndex;"); // used for element! (so we can kill
                // text matched for element)
                /*
                 * println("boolean old_saveConsumedInput=saveConsumedInput;");
                 * if ( !rblk.getAutoGen() ) { // turn off "save input" if ! on
                 * rule println("saveConsumedInput=false;"); }
                 */
            }
            ...
            if (grammar instanceof TreeWalkerGrammar) {
                ...
                println(labeledElementASTType + " " + s.getId()
                        ...
                        + labeledElementASTType + ")_t;", NO_MAPPING);
            }
            if (grammar.buildAST) {
            ...
            // Search for an unlabeled exception specification attached to the
            // rule
                ...
                if (alt.synPred != null) {
                    antlrTool
                            .warning(
                                    ...
                                    grammar.getFilename(), alt.synPred
                                            .getLine(), alt.synPred.getColumn());
                }
                genAlt(alt, rblk);
            } else {
            ...
            // Gen the return statement if there is one (lexer has hard-wired
            // return action)
            if (rblk.returnAction != null) {
                println("return "
            ...
            if (rr.args != null) {
                ...
                if (tInfo.assignToRoot || tInfo.refRuleRoot != null) {
                    antlrTool.error("Arguments of rule reference '"
                            ...
                            + currentRule.getRuleName(), grammar.getFilename(),
                ...
                if (rs.block.argAction == null) {
                    antlrTool.warning("Rule '" + rr.targetRule
                            + "' accepts no arguments", grammar.getFilename(),
                            rr.getLine(), rr.getColumn());
                }
            } else {
                // For C++, no warning if rule has parameters, because there may
                // be default
                // values for all of the parameters
                if (rs.block.argAction != null) {
                    antlrTool.warning(
                            "Missing parameters on reference to rule "
                                    + rr.targetRule, grammar.getFilename(), rr
                                    .getLine(), rr.getColumn());
                }
            }
            _println(");");
                ...
                if (s != null) {
                    if (s.startsWith("\"")) {
                        ...
                        if (sl == null) {
                            ...
                            if (mangledName != null) {
                                // We were able to create a meaningful mangled
                                // token name
                                println("int " + mangledName + " = " + i + ";");
                                // if no label specified, make the label equal
                                // to the mangled name
    ...
    public String getASTCreateString(String astCtorArgs) {
        ...
        if (nCommas < 2) { // if 1 or 2 args
            ...
            if (ts != null) {
                ...
                // System.out.println("node type of "+tokenName+" is
                // "+astNodeType);
                String emptyText = "";
                if (nCommas == 0) {
     ...
     * 
     * @param printWriterManager
     *            the new manager
     */
    public void setPrintWriterManager(
