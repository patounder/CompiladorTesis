 ...
 *
 * $Id: DocBookCodeGenerator.java,v 1.1 2007-10-19 15:55:01 soto-r Exp $
 */
...
public class DocBookCodeGenerator extends CodeGenerator {
    ...
    protected AlternativeElement prevAltElem = null; // what was generated

    // last?

    /**
     * Create a Diagnostic code-generator using the given Grammar The caller
     * must still call setTool, setBehavior, and setAnalyzer before generating
     * code.
     */
    ...
    /**
     * Encode a string for printing in a HTML document.. e.g. encode '<' '>'
     * and similar stuff
     * 
     * @param s
    ...
    public void gen() {
        ...
        try {
            ...
            while (grammarIter.hasMoreElements()) {
                ...
                /*
                 * g.setGrammarAnalyzer(analyzer); analyzer.setGrammar(g);
                 */
    ...
    public void gen(LexerGrammar g) throws IOException {
        setGrammar(g);
        antlrTool.reportProgress("Generating " + grammar.getClassName()
                + ".sgml");
        currentOutput = antlrTool.openOutputFile(grammar.getClassName()
                + ".sgml");
        ...
        println("<para>Definition of lexer " + grammar.getClassName()
        ...
        /*
         * // Generate string literals println(""); println("*** String literals
         * used in the parser"); println("The following string literals were
         * used in the parser."); println("An actual code generator would
         * arrange to place these literals"); println("into a table in the
         * generated lexer, so that actions in the"); println("generated lexer
         * could match token text against the literals."); println("String
         * literals used in the lexer are not listed here, as they");
         * println("are incorporated into the mainstream lexer processing.");
         * tabs++; // Enumerate all of the symbols and look for string literal
         * symbols Enumeration ids = grammar.getSymbols(); while (
         * ids.hasMoreElements() ) { GrammarSymbol sym =
         * (GrammarSymbol)ids.nextElement(); // Only processing string literals --
         * reject other symbol entries if ( sym instanceof StringLiteralSymbol ) {
         * StringLiteralSymbol s = (StringLiteralSymbol)sym; println(s.getId() + " = " +
         * s.getTokenType()); } } tabs--; println("*** End of string literals
         * used by the parser");
         */
    ...
    public void gen(ParserGrammar g) throws IOException {
        ...
        println("<para>Definition of parser " + grammar.getClassName()
                + ", which is a subclass of " + grammar.getSuperClass()
                + ".</para>");
    ...
    public void gen(TreeWalkerGrammar g) throws IOException {
        ...
        // println("This action will appear before the declaration of your
        // tree-walker class:");
        // tabs++;
        // println(grammar.preambleAction.getText());
        // tabs--;
        ...
        println("<para>Definition of tree parser " + grammar.getClassName()
                + ", which is a subclass of " + grammar.getSuperClass()
                + ".</para>");
        ...
        // println("These are the member declarations that you defined for your
        // class:");
        // tabs++;
        // printAction(grammar.classMemberAction.getText());
        // tabs--;
    ...
    public void gen(WildcardElement wc) {
        /*
         * if ( wc.getLabel()!=null ) { _print(wc.getLabel()+"="); }
         */
        _print(". ");
    }
     ...
     * AlternativeBLock. This generates any variable declarations, init-actions,
    ...
    protected void genHeader() {
        ...
        println("  <othercredit>");
        println("    <contrib>");
        println("    Generated by <ulink url=\"http://www.ANTLR.org/\">ANTLR</ulink>"
                + antlrTool.version);
    ...
    /**
     * Generate the nextToken rule. nextToken is a synthetic lexer rule that is
     * the implicit OR of all user-defined lexer rules.
     */
    public void genNextToken() {
        ...
        RuleBlock blk = MakeGrammar.createNextTokenRule(grammar, grammar.rules,
                "nextToken");
        ...
        /*
         * // Analyze the synthesized block if
         * (!grammar.theLLkAnalyzer.deterministic(blk)) { println("The grammar
         * analyzer has determined that the synthesized"); println("nextToken
         * rule is non-deterministic (i.e., it has ambiguities)"); println("This
         * means that there is some overlap of the character");
    ...
    /**
     * Generate the syntactic predicate. This basically generates the
     * alternative block, buts tracks if we are inside a synPred
     * 
     * @param blk
    ...
    protected void genTokenTypes(TokenManager tm) throws IOException {
        ...
        currentOutput = antlrTool.openOutputFile(tm.getName()
                + TokenTypesFileSuffix + TokenTypesFileExt);
        // SAS: changed for proper text file io
        tabs = 0;
     ...
     * 
     * @param depth
     *            The depth of the entire lookahead/follow
     * @param k
     *            The lookahead level to print
     * @param lookahead
    ...
    public void printSet(int depth, int k, Lookahead lookahead) {
        ...
        for (int i = 0; i < elems.length; i++) {
            ...
            if (doingLexRules) {
                ...
                _print((String) grammar.tokenManager.getVocabulary().elementAt(
                        elems[i]));
            }
            if (i != elems.length - 1) {
                _print(", ");
