 ...
 *
 * $Id: DefineGrammarSymbols.java,v 1.1 2007-10-19 15:55:01 soto-r Exp $
 */
...
/**
 * DefineGrammarSymbols is a behavior for the ANTLRParser that adds all the
 * token and rule symbols to the grammar symbol table.
 * 
 * Token types are assigned to token symbols in this class also. The token type
 * for a token is done in the order seen (lexically).
 */
public class DefineGrammarSymbols implements ANTLRGrammarParseBehavior {
    ...
    Token thePreambleAction = new CommonToken(Token.INVALID_TYPE, ""); // init
    ...
    public DefineGrammarSymbols(Tool tool_, String[] args_,
    ...
    public void _refStringLiteral(Token lit, Token label, int autoGenType,
            boolean lastInRule) {
    ...
    /** Reference a token */
    public void _refToken(Token assignId, Token t, Token label, Token args,
    ...
    public void defineRuleName(Token r, String access, boolean ruleAutoGen,
        ...
        if (grammar.isDefined(id)) {
            ...
            if (rs.isDefined()) {
                tool.error("redefinition of rule " + id, grammar.getFilename(),
    ...
    /**
     * Define a token from tokens {...}. Must be label and literal or just label
     * or just a literal.
     */
    public void defineToken(Token tokname, Token tokliteral) {
        ...
        if (literal != null) {
            ...
            if (sl != null) {
                ...
                if (name == null || sl.getLabel() != null) {
                    tool.warning("Redefinition of literal in tokens {...}: "
                            + literal, grammar.getFilename(), tokliteral
                            .getLine(), tokliteral.getColumn());
                    return;
                } else if (name != null) {
            ...
            if (name != null) {
                ...
                if (ts != null) {
                    ...
                    if (ts instanceof StringLiteralSymbol) {
                        tool.warning("Redefinition of token in tokens {...}: "
                                + name, grammar.getFilename(), tokliteral
                                .getLine(), tokliteral.getColumn());
                        return;
                    }
        ...
        // create a token in the token manager not a literal
            ...
            if (grammar.tokenManager.tokenDefined(name)) {
                tool.warning("Redefinition of token in tokens {...}: " + name,
                        grammar.getFilename(), tokname.getLine(), tokname
                                .getColumn());
                return;
            }
            int tt = grammar.tokenManager.nextTokenType();
     ...
     * may not have been set. This method is bigger than it needs to be, but is
     * much more clear if I delineate all the cases.
     */
    public void endOptions() {
        ...
        if (grammar.exportVocab == null && grammar.importVocab == null) {
            ...
            if (tokenManagers.containsKey(DEFAULT_TOKENMANAGER_NAME)) {
                ...
                TokenManager tm = (TokenManager) tokenManagers
                        .get(DEFAULT_TOKENMANAGER_NAME);
                // System.out.println("No tokenVocabulary for '" +
                // grammar.getClassName() + "', using default '" + tm.getName()
                // + "'");
                grammar.setTokenManager(tm);
                return;
            }
            // no shared vocab for file, make new one
            // System.out.println("No exportVocab for '" +
            // grammar.getClassName() + "', creating default '" +
            // grammar.exportVocab + "'");
        ...
        if (grammar.exportVocab == null && grammar.importVocab != null) {
            ...
            if (tokenManagers.containsKey(grammar.importVocab)) {
                ...
                TokenManager tm = (TokenManager) tokenManagers
                        .get(grammar.importVocab);
                // System.out.println("Duping importVocab of " +
                // grammar.importVocab);
            ...
            ImportVocabTokenManager tm = new ImportVocabTokenManager(grammar,
                    grammar.importVocab + CodeGenerator.TokenTypesFileSuffix
                            + CodeGenerator.TokenTypesFileExt,
                    grammar.exportVocab, tool);
            tm.setReadOnly(false); // since renamed, can write out
            // Add this token manager to the list so its tokens will be
            // generated
            tokenManagers.put(grammar.exportVocab, tm);
            // System.out.println("vocab renamed to default output vocab of
            // "+tm.getName());
        ...
        if (grammar.exportVocab != null && grammar.importVocab == null) {
            ...
            if (tokenManagers.containsKey(grammar.exportVocab)) {
                // Use the already-defined token manager
                TokenManager tm = (TokenManager) tokenManagers
                        .get(grammar.exportVocab);
                // System.out.println("Sharing exportVocab of " +
                // grammar.exportVocab);
                grammar.setTokenManager(tm);
                return;
            }
            // create new output vocab
            // System.out.println("Creating exportVocab " +
            // grammar.exportVocab);
            TokenManager tm = new SimpleTokenManager(grammar.exportVocab, tool);
            grammar.setTokenManager(tm);
            // Add the token manager to the list of token managers
    ...
    public void refTokenRange(Token t1, Token t2, Token label, int autoGenType,
            boolean lastInRule) {
        // ensure that the DefineGrammarSymbols methods are called; otherwise a
        // range addes more
        ...
        if (t1.getText().charAt(0) == '"') {
            ...
            _refToken(null, t1, null, null, false,
                    GrammarElement.AUTO_GEN_NONE, lastInRule);
        }
        if (t2.getText().charAt(0) == '"') {
            _refStringLiteral(t2, null, GrammarElement.AUTO_GEN_NONE,
                    lastInRule);
        } else {
            _refToken(null, t2, null, null, false,
                    GrammarElement.AUTO_GEN_NONE, lastInRule);
        }
    }
    ...
    /**
     * setFileOption: Associate an option value with a key. This applies to
     * options for an entire grammar file.
     * 
     * @param key
    ...
    public void setFileOption(Token key, Token value, String filename) {
        if (key.getText().equals("language")) {
            if (value.getType() == ANTLRParser.STRING_LITERAL) {
                language = StringUtils.stripBack(StringUtils.stripFront(value
                        .getText(), '"'), '"');
                ...
                tool.error("language option must be string or identifier",
                        filename, value.getLine(), value.getColumn());
            }
        } else if (key.getText().equals("mangleLiteralPrefix")) {
            if (value.getType() == ANTLRParser.STRING_LITERAL) {
                tool.literalsPrefix = StringUtils.stripFrontBack(value
                        .getText(), "\"", "\"");
            } else {
            ...
            if (!language.equals("Cpp")) {
                ...
                if (key.getText().equals("noConstructors")) {
                    if (!(value.getText().equals("true") || value.getText()
                            .equals("false")))
                        tool.error(
            ...
            tool.error("Invalid file-level option: " + key.getText(), filename,
                    key.getLine(), value.getColumn());
        }
    }

    /**
     * setGrammarOption: Associate an option value with a key. This function
     * forwards to Grammar.setOption for some options.
     * 
     * @param key
     *            The token containing the option name
     * @param value
        ...
        if (key.getText().equals("tokdef")
            ...
            tool
                    .error(
                            ...
                            grammar.getFilename(), value.getLine(), value
                                    .getColumn());
        } else if (key.getText().equals("literal")
                && grammar instanceof LexerGrammar) {
            tool.error("the literal option is invalid >= ANTLR 2.6.0.\n"
                    + "  Use the \"tokens {...}\" mechanism instead.", grammar
                    .getFilename(), value.getLine(), value.getColumn());
            ...
            if (value.getType() == ANTLRParser.RULE_REF
                ...
                tool.error("exportVocab must be an identifier", grammar
                        .getFilename(), value.getLine(), value.getColumn());
            }
            ...
            if (value.getType() == ANTLRParser.RULE_REF
                ...
                tool.error("importVocab must be an identifier", grammar
                        .getFilename(), value.getLine(), value.getColumn());
            }
        } else if (key.getText().equals("k")) {
            if (grammar instanceof TreeWalkerGrammar
                    && !value.getText().equals("1")) {
                tool.error("Treewalkers only support k=1", grammar
                        .getFilename(), value.getLine(), value.getColumn());
            } else {
    ...
    public void startParser(String file, Token name, String superClass,
            String doc) {
        if (numParsers > 0) {
            tool.panic("You may only have one parser per grammar file: class "
                    + name.getText());
        }
        numParsers++;
        reset();
    ...
    public void startTreeWalker(String file, Token name, String superClass,
        ...
        if (numTreeParsers > 0) {
            tool
                    .panic("You may only have one tree parser per grammar file: class "
                            + name.getText());
        }
        numTreeParsers++;
        reset();
        ...
        if (g != null) {
            if (!(g instanceof TreeWalkerGrammar)) {
                ...
                tool.panic("Tree-walker '" + name.getText()
                        + "' is already defined");
            }
        } else {
            // Create a new grammar
