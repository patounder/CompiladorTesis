 ...
 *
 * $Id: Parser.java,v 1.1 2007-10-19 15:55:01 soto-r Exp $
 */
...
/**
 * A generic ANTLR parser (LL(k) for k>=1) containing a bunch of utility
 * routines useful at any lookahead depth. We distinguish between the LL(1) and
 * LL(k) parsers because of efficiency. This may not be necessary in the near
 * future.
 * 
 * Each parser object contains the state of the parse including a lookahead
 * cache (the form of which is determined by the subclass), whether or not the
 * parser is in guess mode, where tokens come from, etc...
 * 
 * <p>
 * During <b>guess</b> mode, the current lookahead token(s) and token type(s)
 * cache must be saved because the token stream may not have been informed to
 * save the token (via <tt>mark</tt>) before the <tt>try</tt> block.
...
public abstract class Parser {
    ...
    /**
     * AST support code; parser delegates to this object. This is set during
     * parser construction by default to either "new ASTFactory()" or a ctor
     * that has a token type to class map for hetero nodes.
     */
    protected ASTFactory astFactory = null;

    /**
     * Constructed if any AST types specified in tokens{..}. Maps an
    ...
    /**
     * If the user specifies a tokens{} section with heterogeneous AST node
     * types, then ANTLR generates code to fill this mapping.
     */
    ...
    public AST getAST() {
        // System.out.println("en Parser getAST");
        return returnAST;
    }
    ...
    /**
     * Return the token type of the ith token of lookahead where i=1 is the
     * current token being examined by the parser (i.e., it has not been matched
     * yet).
     */
     ...
     * Throw an exception upon mismatch, which is catch by either the error
     * handler or by the syntactic predicate.
     */
    public void match(int t) throws MismatchedTokenException,
            TokenStreamException {
        // System.out.println("t: "+t +" LA: "+ LA(1));
        // System.out.println("t: "+t +" LA: "+ LA(1));
        if (LA(1) != t) {
            // System.out.println("error t: "+t +" LA: "+ LA(1));
    ...
    /**
     * Make sure current lookahead symbol matches the given set Throw an
     * exception upon mismatch, which is catch by either the error handler or by
     ...
     *             directly to stderr, which is usually not appropriate when a
     *             parser is embedded into a larger application. Since the
     *             method is <code>static</code>, it cannot be overridden to
     ...
     * Normally, the programmer does not play with this, using setASTNodeType
     * instead.
     */
    ...
    /**
     * Specify the type of node to create during tree building; use
     * setASTNodeClass now to be consistent with Token Object Type accessor.
     * 
