...
/**
 * This token stream tracks the *entire* token stream coming from a lexer, but
 * does not pass on the whitespace (or whatever else you want to discard) to the
 * parser.
 * 
 * This class can then be asked for the ith token in the input stream. Useful
 * for dumping out the input stream exactly after doing some augmentation or
 * other manipulations. Tokens are index from 0..n-1
 * 
...
public class TokenStreamRewriteEngine implements TokenStream, IASDebugStream {
    ...
    public void replace(String programName, Token from, Token to, String text) {
        replace(programName, ((TokenWithIndex) from).getIndex(),
    ...
    public String toString(String programName, int start, int end) {
        ...
        while (tokenCursor >= MIN_TOKEN_INDEX && tokenCursor <= end
            ...
            if (rewriteOpIndex < rewrites.size()) {
                RewriteOperation op = (RewriteOperation) rewrites
                        .get(rewriteOpIndex);
                while (tokenCursor == op.index
                        && rewriteOpIndex < rewrites.size()) {
                    /*
                     * System.out.println("execute op "+rewriteOpIndex+ " (type
                     * "+op.getClass().getName()+")" +" at index "+op.index);
                     */
