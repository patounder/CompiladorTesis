 ...
 *
 * $Id: CodeGenerator.java,v 1.1 2007-10-19 15:55:02 soto-r Exp $
 */
 ...
 * itself living inside of a Grammar object (in which case, this class generates
 * only one recognizer). The main method to call is <tt>gen()</tt>, which
 * initiates all code gen.
 * 
 * <p>
 * The interaction of the code generator with the analyzer is simple: each
 * subrule block calls deterministic() before generating code for the block.
 * Method deterministic() sets lookahead caches in each Alternative object.
 * Technically, a code generator doesn't need the grammar analyzer if all
 * lookahead analysis is done at runtime, but this would result in a slower
 * parser.
 * 
 * <p>
 * This class provides a set of support utilities to handle argument list
 * parsing and so on.
 * 
...
public abstract class CodeGenerator {
    ...
    /**
     * Object used to format characters in the target language. subclass must
     * initialize this to the language-specific formatter
     */
     ...
     * language-specific equivalent will be generated instead of a series of
     * if/else statements for blocks with number of alternates greater than or
     * equal to this number of non-predicated LL(1) alternates. This is modified
     * by the grammar option "codeGenMakeSwitchThreshold"
     */
    protected int makeSwitchThreshold = DEFAULT_MAKE_SWITCH_THRESHOLD;

    /**
     * This is a hint for the language-specific code generator. A bitset
     * membership test will be generated instead of an ORed series of LA(k)
     * comparisions for lookahead sets with degree greater than or equal to this
     * value. This is modified by the grammar option
     ...
     * 
     * @param elems
     *            The array of elements representing the set, usually from
     *            BitSet.toArray().
    ...
    /**
     * Get the identifier portion of an argument-action token. The ID of an
     * action is assumed to be a trailing identifier. Specific code-generators
     * may want to override this if the language has unusual declaration syntax.
     * 
     * @param t
     *            The action token
    ...
    /**
     * Get the identifier portion of an argument-action. The ID of an action is
     * assumed to be a trailing identifier. Specific code-generators may want to
     * override this if the language has unusual declaration syntax.
     * 
     * @param s
     *            The action text
     * @param line
    ...
    protected String extractIdOfAction(String s, int line, int column) {
        ...
        antlrTool.warning("Ill-formed action", grammar.getFilename(), line,
                column);
        return "";
    }

    /**
     * Get the type string out of an argument-action token. The type of an
     * action is assumed to precede a trailing identifier Specific
     * code-generators may want to override this if the language has unusual
     * declaration syntax.
     * 
     * @param t
     *            The action token
    ...
    /**
     * Get the type portion of an argument-action. The type of an action is
     * assumed to precede a trailing identifier Specific code-generators may
     * want to override this if the language has unusual declaration syntax.
     * 
     * @param s
     *            The action text
     * @param line
    ...
    /**
     * Generate the token types as a text file for persistence across shared
     * lexer/parser
     */
     ...
     * @param forInput
     *            true if the input tree node variable is to be returned,
     *            otherwise the output variable is returned.
    ...
    /**
     * Add a bitset to the list of bitsets to be generated. if the bitset is
     * already in the list, ignore the request. Always adds the bitset to the
     * end of the list, so the caller can rely on the position of bitsets in the
     * list. The returned position can be used to format the bitset name, since
     * it is invariant.
     * 
     * @param p
     *            Bit set to mark for code generation
     * @param forParser
     *            true if the bitset is used for the parser, false for the lexer
    ...
    /**
     * Print an action with leading tabs, attempting to preserve the current
     * indentation level for multi-line actions Ignored if string is null.
     * 
     * @param s
     *            The action string to output
    ...
    /**
     * Lexically process $ and # references within the action. This will replace
     * #id and #(...) with the appropriate function calls and/or variables
     * etc...
     */
    protected abstract String processActionForSpecialSymbols(String actionStr,
            int line, RuleBlock currentRule, ActionTransInfo tInfo);
    ...
    protected void setGrammar(Grammar g) {
        ...
        if (grammar.hasOption("codeGenMakeSwitchThreshold")) {
            try {
                makeSwitchThreshold = grammar
                        .getIntegerOption("codeGenMakeSwitchThreshold");
                // System.out.println("setting codeGenMakeSwitchThreshold to " +
                // makeSwitchThreshold);
                ...
                antlrTool
                        .error(
                                ...
                                grammar.getClassName(), tok.getLine(), tok
                                        .getColumn());
            }
        }
        ...
        if (grammar.hasOption("codeGenBitsetTestThreshold")) {
            try {
                bitsetTestThreshold = grammar
                        .getIntegerOption("codeGenBitsetTestThreshold");
                // System.out.println("setting codeGenBitsetTestThreshold to " +
                // bitsetTestThreshold);
            } catch (NumberFormatException e) {
        ...
        if (grammar.hasOption("codeGenDebug")) {
            ...
            if (t.getText().equals("true")) {
                ...
                antlrTool.error("option 'codeGenDebug' must be true or false",
                        grammar.getClassName(), t.getLine(), t.getColumn());
            }
        }
    }
