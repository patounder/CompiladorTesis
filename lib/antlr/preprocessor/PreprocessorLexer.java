...
public class PreprocessorLexer extends antlr.CharScanner implements
    ...
    /**
     * This rule picks off keywords in the lexer that need to be handled
     * specially. For example, "header" is the start of the header action (used
     * to distinguish between options block and an action). We do not want
     * "header" to go back to the parser as a simple keyword...it must pick off
    ...
    public final void mID_OR_KEYWORD(boolean _createToken)
        ...
        _ttype = id.getType();
        {
            ...
            } else if (((_tokenSet_10.member(LA(1))) && ((LA(2) >= '\u0003' && LA(2) <= '\u00ff')))
                    && (id.getText().equals("tokens"))) {
                {
                    _loop333: do {
                        switch (LA(1)) {
                        case '\t':
    ...
    protected final void mID(boolean _createToken) throws RecognitionException,
        ...
        _ttype = testLiteralsTable(new String(text.getBuffer(), _begin, text
                .length()
                - _begin), _ttype);
        if (_createToken && _token == null && _ttype != Token.SKIP) {
            _token = makeToken(_ttype);
