 ...
 *
 * $Id: RuleBlock.java,v 1.1 2007-10-19 15:54:56 soto-r Exp $
 */
...
public class RuleBlock extends AlternativeBlock {
    ...
    // 1..k
    protected Lookahead cache[];// Each rule can cache it's lookahead
    ...
    public void addExceptionSpec(ExceptionSpec ex) {
        if (findExceptionSpec(ex.label) != null) {
            if (ex.label != null) {
                grammar.antlrTool.error("Rule '" + ruleName
            ...
            exceptionSpecs
                    .put((ex.label == null ? "" : ex.label.getText()), ex);
        }
    }
    ...
    public void setOption(Token key, Token value) {
        if (key.getText().equals("defaultErrorHandler")) {
            ...
            if (!(grammar instanceof LexerGrammar)) {
                grammar.antlrTool.error(
                        "ignore option only valid for lexer rules", grammar
            ...
            if (!(grammar instanceof LexerGrammar)) {
                grammar.antlrTool.error(
                        "paraphrase option only valid for lexer rules", grammar
                                .getFilename(), key.getLine(), key.getColumn());
            } else {
                ...
                if (ts == null) {
                    grammar.antlrTool
                            .panic("cannot find token associated with rule "
                                    + ruleName);
                }
