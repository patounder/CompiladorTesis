 ...
 *
 * $Id: TokenStreamHiddenTokenFilter.java,v 1.1 2007-10-19 15:55:01 soto-r Exp $
 */

import antlr.collections.impl.BitSet;

/**
 * This object filters a token stream coming from a lexer or another TokenStream
 * so that only certain token channels get transmitted to the parser.
 * 
 * Any of the channels can be filtered off as "hidden" channels whose tokens can
 * be accessed from the parser.
 */
public class TokenStreamHiddenTokenFilter extends TokenStreamBasicFilter
    ...
    /**
     * Return a ptr to the hidden token appearing immediately after token t in
     * the input stream.
     */
    ...
    /**
     * Return a ptr to the hidden token appearing immediately before token t in
     * the input stream.
     */
    ...
    /**
     * Return the next monitored token. Test the token following the monitored
     * token. If following is another monitored token, save it for the next
     * invocation of nextToken (like a single lookahead token) and return it
     * then. If following is unmonitored, nondiscarded (hidden) channel token,
    ...
    public Token nextToken() throws TokenStreamException {
        ...
        while (hideMask.member(LA(1).getType())
            ...
            if (hideMask.member(LA(1).getType())) {
                ...
                p.setHiddenAfter(LA(1));
                // link backwards
                if (p != monitored) { // hidden cannot point to monitored
                    // tokens
