 ...
 *
 * $Id: CharScanner.java,v 1.1 2007-10-19 15:54:58 soto-r Exp $
 */
...
public abstract class CharScanner implements TokenStream {
    ...
    protected boolean saveConsumedInput = true; // does consume() save
    ...
    /**
     * Tab chars are handled by tab() according to this value; override method
     * to do anything weird with tabs.
     */
    protected int tabsize = 8;

    protected Token _returnToken = null; // used to return tokens w/o using
    ...
    /**
     * Used during filter mode to indicate that path is desired. A subsequent
     * scan error will report an error as usual if acceptPath=true;
     */
    ...
    public char[] LCOLA() throws CharStreamException {
        return inputState.input.LCOLA();
    }
    ...
    protected Token makeToken(int t) {
        try {
            ...
            tok.setFilename(inputState.filename);
    ...
    public void match(String s) throws MismatchedCharException,
        ...
        for (int i = 0; i < len; i++) {
            if (LA(1) != s.charAt(i)) {
                throw new MismatchedCharException(LA(1), s.charAt(i), false,
                        this);
            }
            consume();
        }
    ...
    /**
     * advance the current column number by an appropriate amount according to
     * tab size. This method is called from consume().
     */
    public void tab() {
        int c = getColumn();
        int nc = (((c - 1) / tabsize) + 1) * tabsize + 1; // calculate tab
        // stop
        setColumn(nc);
    }
    ...
    /**
     * Test the text passed in against the literals table Override this method
     * to perform a different literals test This is used primarily when you want
     * to test a portion of a token.
     */
    ...
    /**
     * This method is called by YourLexer.nextToken() when the lexer has hit EOF
     * condition. EOF is NOT a character. This method is not called if EOF is
     * reached during syntactic predicate evaluation or during evaluation of
     * normal lexical rules, which presumably would be an IOException. This
     * traps the "normal" EOF condition.
     * 
     * uponEOF() is called after the complete evaluation of the previous token
     * and only if your parser asks for another token beyond that last non-EOF
     * token.
     * 
     * You might want to throw token or char stream exceptions like: "Heh,
     * premature eof" or a retry stream exception ("I found the end of this
     * file, go back to referencing file").
     */
