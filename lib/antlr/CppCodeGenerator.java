 ...
 *
 * $Id: CppCodeGenerator.java,v 1.1 2007-10-19 15:55:02 soto-r Exp $
 */
...
/**
 * Generate MyParser.cpp, MyParser.hpp, MyLexer.cpp, MyLexer.hpp and
 * MyParserTokenTypes.hpp
 */
public class CppCodeGenerator extends CodeGenerator {
    ...
    String labeledElementASTType; // mostly the same as labeledElementType
    ...
    /**
     * Print an action without leading tabs, attempting to preserve the current
     * indentation level for multi-line actions Ignored if string is null.
     * 
     * @param s
    ...
    public void printAction(Token t) {
        if (t != null) {
            genLineNo(t.getLine());
            printTabs();
            _printAction(processActionForSpecialSymbols(t.getText(), t
                    .getLine(), null, null));
            genLineNo2();
        }
    }
    ...
    /**
     * Output a String followed by newline, to the currentOutput stream. Ignored
     * if string is null.
     * 
     * @param s
     *            The string to output
    ...
    /**
     * Output tab indent followed by a String followed by newline, to the
     * currentOutput stream. Ignored if string is null.
     * 
     * @param s
     *            The string to output
     */
    ...
    /**
     * Normalize a string coming from antlr's lexer. E.g. translate java escapes
     * to values. Check their size (multibyte) bomb out if they are multibyte
     * (bit crude). Then reescape to C++ style things. Used to generate strings
     * for match() and matchRange()
     * 
     * @param lit
     *            the literal string
     * @param isCharLiteral
     *            if it's for a character literal (enforced to be one length)
     *            and enclosed in ' FIXME: bombing out on mb chars. Should be
     *            done in Lexer. FIXME: this is another horrible hack. FIXME:
     *            life would be soooo much easier if the stuff from the lexer
     *            was normalized in some way.
     */
    private String convertJavaToCppString(String lit, boolean isCharLiteral) {
        ...
        while (i < s.length()) {
            if (s.charAt(i) == '\\') {
                ...
                case 'u':
                    ...
                    if (i + 5 < s.length()) {
                        val = Character.digit(s.charAt(i + 2), 16) * 16 * 16
                                * 16 + Character.digit(s.charAt(i + 3), 16)
                                * 16 * 16
                        ...
                        antlrTool
                                .error("Invalid escape in char literal: '"
                                        + lit + "' looking at '"
                                        + s.substring(i) + "'");
                    break;
                ...
                default:
                    antlrTool.error("Unhandled escape in char literal: '" + lit
                            + "' looking at '" + s.substring(i) + "'");
                    val = 0;
                }
            } else
            ...
            if (isCharLiteral) {
                ...
                if (maxsize <= 255) {
                    if ((val <= 255) && (val & 0x80) != 0)
                        ...
                        // actually the support lib needs to be fixed but that's
                        // a bit
                        // hairy too.
                        ret = "static_cast<unsigned char>('"
                                + charFormatter.escapeChar(val, true) + "')";
                    else
    ...
    public void gen() {
        ...
        try {
            ...
            while (grammarIter.hasMoreElements()) {
                ...
                // To get right overloading behavior across hetrogeneous
                // grammars
    ...
    public void gen(CharLiteralElement atom) {
        ...
        if (!(grammar instanceof LexerGrammar))
            antlrTool
                    .error("cannot ref character literals in grammar: " + atom);
    ...
    public void gen(CharRangeElement r) {
        ...
        boolean save = (grammar instanceof LexerGrammar && (!saveText || r
    ...
    public void gen(OneOrMoreBlock blk) {
        ...
        genBlockFinish(howToFinish, "if ( " + cnt + ">=1 ) { goto " + label
                + "; } else {" + throwNoViable + "}");
    ...
    public void gen(TreeElement t) {
        ...
        if (t.root.getLabel() != null) {
            println(t.root.getLabel() + " = (_t == " + labeledElementType
                    + "(ASTNULL)) ? " + labeledElementASTInit + " : _t;");
        }
    ...
    public void gen(WildcardElement wc) {
        ...
        if (grammar instanceof TreeWalkerGrammar) {
            ...
            if (grammar instanceof LexerGrammar
                    && (!saveText || wc.getAutoGenType() == GrammarElement.AUTO_GEN_BANG)) {
                println("text.erase(_saveIndex);"); // kill text atom put in
                // buffer
            }
        } else {
    ...
    protected void genAlt(Alternative alt, AlternativeBlock blk) {
        ...
        if (genAST) {
            if (blk instanceof RuleBlock) {
                ...
                // "+labeledElementASTType+"(currentAST.root);");
                antlrTool.warning("Labeled subrules are not implemented",
    ...
    /**
     * Generate the initaction for a block, which may be a RuleBlock or a plain
     * AlternativeBLock.
     * 
        ...
        if (blk instanceof RuleBlock) {
            ...
            if (rblk.labeledElements != null) {
                for (int i = 0; i < rblk.labeledElements.size(); i++) {
                    ...
                    // Variables for labeled rule refs and subrules are
                    // different than
                    // variables for grammar atoms. This test is a little tricky
                    // because
                    // we want to get all rule refs and ebnf, but not rule
                    // blocks or
                    ...
                    if (a instanceof RuleRefElement
                        ...
                        if (!(a instanceof RuleRefElement)
                            ...
                            if (grammar.buildAST) {
                                // Always gen AST variables for labeled
                                // elements,
                            ...
                            if (grammar instanceof TreeWalkerGrammar) {
                                // always generate rule-ref variables for tree
                                // walker
                                println(labeledElementType + " " + a.getLabel()
                                        + " = " + labeledElementInit + ";");
                            }
    ...
    public void genBody(LexerGrammar g) throws IOException {
        ...
        if (grammar.debuggingOutput) {
            println("const char* " + grammar.getClassName()
                    + "::_ruleNames[] = {");
            tabs++;
    ...
    public void genInitFactory(Grammar g) {
        ...
        if (g.buildAST) {
            ...
            while (tokens.hasMoreElements()) {
                ...
                // if we have a custom type and there's not a more local
                // override
                ...
                if (ts.getASTNodeType() != null) {
                    ...
                    String type = (String) astTypes
                            .elementAt(ts.getTokenType());
                    if (type == null)
                        astTypes.setElementAt(ts.getASTNodeType(), ts
                                .getTokenType());
                    else {
                        // give a warning over action taken if the types are
                        // unequal
                        if (!ts.getASTNodeType().equals(type)) {
                            antlrTool.warning("Token " + tok
                                    + " taking most specific AST type", grammar
                                    .getFilename(), 1, 1);
                            antlrTool.warning("  using " + type + " ignoring "
                                    + ts.getASTNodeType(), grammar
                                    .getFilename(), 1, 1);
                        }
                    }
                }
            }
            // now actually write out all the registered types. (except the
            // default
            // type.
    ...
    public void genBody(ParserGrammar g) throws IOException {
        ...
        while (ids.hasMoreElements()) {
            ...
            if (sym instanceof RuleSymbol) {
                ...
                genRule(rs, rs.references.size() == 0, ruleNum++, grammar
                        .getClassName()
                        + "::");
            }
            exitIfError();
        ...
        genBitsets(bitsetsUsed, grammar.tokenManager.maxTokenType(), grammar
                .getClassName()
                + "::");
    ...
    public CppBlockFinishingInfo genCommonBlock(AlternativeBlock blk,
        ...
        if (nLL1 >= makeSwitchThreshold) {
            ...
            for (int i = 0; i < blk.alternatives.size(); i++) {
                ...
                // ignore any non-LL(1) alts, predicated alts or end-of-token
                // alts
        ...
        // generate the
        // alternatives containing trailing end-of-token in their lookahead sets
        // *after*
        ...
        for (int altDepth = startDepth; altDepth >= 0; altDepth--) {
            ...
            for (int i = 0; i < blk.alternatives.size(); i++) {
                ...
                if (grammar instanceof LexerGrammar) {
                    // Calculate the "effective depth" of the alt, which is the
                    // max
                    ...
                    // Ignore alts whose effective depth is other than the ones
                    // we
                ...
                if (alt.cache[1].fset.degree() > caseSizeThreshold
                    ...
                    if (alt.semPred != null) {
                        ...
                        // ignore translation info...we don't need to do
                        // anything with it.

                        // call that will inform SemanticPredicateListeners of
                        // the
                        // result
                        if (grammar.debuggingOutput
        ...
        if (createdLL1Switch) {
            ...
            // return new CppBlockFinishingInfo(ps+"}",true,nIF>0); // close up
            // switch statement
            ...
            if (needASTDecl) {
                if (el instanceof GrammarAtom) {
                    ...
                    if (ga.getASTNodeType() != null) {
                        genASTDeclaration(el, astNameBase, "Ref"
                                + ga.getASTNodeType());
    ...
    public void genInclude(LexerGrammar g) throws IOException {
        ...
        println(", public " + grammar.tokenManager.getName()
        ...
        if (grammar.classMemberAction != null) {
            ...
            print(processActionForSpecialSymbols(grammar.classMemberAction
                    .getText(), grammar.classMemberAction.getLine(),
                    currentRule, null));
            genLineNo2();
        }
        ...
        // Generate the bitsets used throughout the lexer
        genBitsetsHeader(bitsetsUsed, ((LexerGrammar) grammar).charVocabulary
                .size());
    ...
    protected void genASTDeclaration(AlternativeElement el, String var_name,
        ...
        if (el instanceof GrammarAtom
            ...
            init = "Ref" + ((GrammarAtom) el).getASTNodeType() + "("
                    + labeledElementASTInit + ")";
    ...
    protected void genMatchUsingAtomText(GrammarAtom atom) {
        ...
        if (atom.atomText.equals("EOF")) {
            ...
            if (grammar instanceof LexerGrammar) // lexer needs special
            // handling
            {
    ...
    public void genNextToken() {
        ...
        if (!hasPublicRules) {
            ...
            println(namespaceAntlr + "RefToken " + grammar.getClassName()
                    + "::nextToken() { return " + namespaceAntlr
                    + "RefToken(new " + namespaceAntlr + "CommonToken("
                    + namespaceAntlr + "Token::EOF_TYPE, \"\")); }");
            println("");
            return;
        }
        ...
        if (((LexerGrammar) grammar).filterMode) {
            if (filterRule == null) {
                ...
                errFinish += "else {" + newline + "\t\t\t\t\tcommit();"
                        + newline + "\t\t\t\t\ttry {m" + filterRule
                        + "(false);}" + newline + "\t\t\t\t\tcatch("
                        + namespaceAntlr + "RecognitionException& e) {"
                        + newline + "\t\t\t\t\t	// catastrophic failure"
                        + newline + "\t\t\t\t\t	reportError(e);" + newline
                        + "\t\t\t\t\t	consume();" + newline + "\t\t\t\t\t}"
                        + newline + "\t\t\t\t\tgoto tryAgain;" + newline
                        + "\t\t\t\t}";
            }
        } else {
        ...
        if (((LexerGrammar) grammar).filterMode) {
            if (filterRule == null) {
                ...
                println("catch(" + namespaceAntlr
                        + "RecognitionException& ee) {");
     ...
     * alternative unless the rule or the alternative have a '!' modifier.
     * 
     * If an alternative defeats the default tree construction, it must set
     * <rule>_AST to the root of the returned AST.
     * 
     * Each alternative that does automatic tree construction, builds up root
     * and child list pointers in an ASTPair structure.
     * 
    ...
    public void genRule(RuleSymbol s, boolean startSymbol, int ruleNum,
        ...
        if (rblk.returnAction != null) {
            ...
            _print(extractTypeOfAction(rblk.returnAction, rblk.getLine(), rblk
                    .getColumn())
                    + " ");
        } else {
        ...
        if (rblk.argAction != null) {
            ...
            tabs--;
            print(") ");
            // genLineNo2(); // gcc gives error on the brace... hope it works
            // for the others too
        } else {
        ...
        if (grammar instanceof LexerGrammar) {
            ...
            println(namespaceStd + "string::size_type _saveIndex;"); // used
            // for
            // element!
            // (so
            // we
        ...
        if (grammar instanceof TreeWalkerGrammar) {
            ...
            println(labeledElementASTType + " " + s.getId()
                    + "_AST_in = (_t == " + labeledElementASTType
                    + "(ASTNULL)) ? " + labeledElementASTInit + " : _t;");
        }
        if (grammar.buildAST) {
            ...
            println(namespaceAntlr + "ASTPair currentAST;"); // = new
            // ASTPair();");
        ...
        if (rblk.returnAction != null) {
            println("return "
                    + extractIdOfAction(rblk.returnAction, rblk.getLine(), rblk
                            .getColumn()) + ";");
        }
    ...
    private void GenRuleInvocation(RuleRefElement rr) {
        ...
        if (rr.args != null) {
            ...
            ActionTransInfo tInfo = new ActionTransInfo();
            // FIXME: fix line number passed to processActionForTreeSpecifiers
            // here..
            ...
            // if (rs.block.argAction != null) {
            // tool.warning("Missing parameters on reference to rule
            // "+rr.targetRule, rr.getLine());
            // }
        }
        _println(");");
    ...
    protected void genTokenTypes(TokenManager tm) throws IOException {
        ...
        // Generate a guard wrapper
        println("#endif /*INC_" + tm.getName() + TokenTypesFileSuffix
                + "_hpp_*/");
        ...
        if (atom != null && atom.getASTNodeType() != null) {
            ...
            if (type == null)
                ...
                if (!atom.getASTNodeType().equals(type)) {
                    antlrTool.warning("Attempt to redefine AST type for "
                            + atom.getText(), grammar.getFilename(), atom
                            .getLine(), atom.getColumn());
                    antlrTool.warning(" from \"" + type + "\" to \""
                            + atom.getASTNodeType() + "\" sticking to \""
                            + type + "\"", grammar.getFilename(), atom
                            .getLine(), atom.getColumn());
                } else
                    astTypes
                            .setElementAt(atom.getASTNodeType(), atom.getType());
            }
            ...
            if (str.indexOf(',') != -1)
                is_constructor = grammar.tokenManager.tokenDefined(str
                        .substring(0, str.indexOf(',')));

            // System.out.println("getAstCreateString(as): "+str+"
            // "+grammar.tokenManager.tokenDefined(str));
            if (usingCustomAST && (grammar instanceof TreeWalkerGrammar)
                    && !(grammar.tokenManager.tokenDefined(str))
                    && !is_constructor)
    ...
    public String getASTCreateString(String str) {
        // System.out.println("getAstCreateString(str): "+str+"
        // "+grammar.tokenManager.tokenDefined(str));
        if (usingCustomAST)
            return labeledElementASTType + "(astFactory->create("
                    + namespaceAntlr + "RefAST(" + str + ")))";
        else
            return "astFactory->create(" + str + ")";
    }
    ...
    protected String getLookaheadTestExpression(Alternative alt, int maxDepth) {
        ...
        /*
         * boolean first = true; for (int i=1; i<=depth && i<=maxDepth; i++) {
         * BitSet p = alt.cache[i].fset; if (!first) { e.append(") && ("); }
         * first = false; // Syn preds can yield <end-of-syn-pred> (epsilon)
         * lookahead. // There is no way to predict what that token would be.
         * Just // allow anything instead. if ( alt.cache[i].containsEpsilon() ) {
         * e.append("true"); } else { e.append(getLookaheadTestTerm(i, p)); } }
         * 
         * e.append(")");
         */
    ...
    private String getValueString(int value) {
        ...
        if (grammar instanceof LexerGrammar) {
            ...
            if (ts == null) {
                ...
                // tool.panic("vocabulary for token type " + value + " is
                // null");
            }
            String tId = ts.getId();
    ...
    public String mapTreeId(String idParam, ActionTransInfo transInfo) {
        ...
        if (grammar instanceof TreeWalkerGrammar) {
            ...
            // #rulename_in to 'rulename_in' else to 'rulename_AST_in' which
            // indeed
            // exists. disabling for now.. and hope it doesn't blow up
            // somewhere.
            if (!grammar.buildAST) {
                in_var = true;
        ...
        for (int i = 0; i < currentRule.labeledElements.size(); i++) {
            ...
            if (elt.getLabel().equals(id)) {
                // if( in_var )
                // System.out.println("returning (vec) "+(in_var ? id : id +
                // "_AST"));
        ...
        // System.out.println("Last check: "+id+" ==
        // "+currentRule.getRuleName());
    ...
    private void setupGrammarParameters(Grammar g) {
        if (g instanceof ParserGrammar || g instanceof LexerGrammar
            ...
            if (g.hasOption("noConstructors")) {
                ...
                if ((t != null)
                        && !(t.getText().equals("true") || t.getText().equals(
                                "false")))
                    antlrTool.error(
                            "noConstructors option must be true or false",
                            antlrTool.getGrammarFile(), t.getLine(), t
                                    .getColumn());
