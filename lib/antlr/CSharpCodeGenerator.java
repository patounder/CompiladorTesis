 ...
 *
 * $Id: CSharpCodeGenerator.java,v 1.1 2007-10-19 15:54:57 soto-r Exp $
 */
...
public class CSharpCodeGenerator extends CodeGenerator {
    ...
    // _saveIndex creation optimization -- don't create it unless we need to use
    // it
    ...
    /**
     * Create a CSharp code-generator using the given Grammar. The caller must
    ...
    public void gen(ActionElement action) {
        ...
        if (action.isSemPred) {
            ...
            if (tInfo.assignToRoot) {
                ...
                // list
                println("if ( (null != " + tInfo.refRuleRoot + ") && (null != "
    ...
    private void genBlockFinish(CSharpBlockFinishingInfo howToFinish,
        ...
        if (howToFinish.postscript != null) {
            if (howToFinish.needAnErrorClause && howToFinish.generatedSwitch
                ...
                // Check to make sure that noViableAction is only a throw
                // statement
                if (noViableAction.indexOf("throw") == 0
                        || noViableAction.indexOf("goto") == 0) {
                    // Remove the break statement since it isn't reachable with
                    // a throw exception
                    int endOfBreak = howToFinish.postscript.indexOf("break;") + 6;
                    String newPostScript = howToFinish.postscript
                            .substring(endOfBreak);
    ...
    public void genBody(ParserGrammar g) throws IOException {
        ...
        if (usingCustomAST) {
            // when we are using a custom AST, overload Parser.getAST() to
            // return the
            // custom AST type
    ...
    public void genBody(TreeWalkerGrammar g) throws IOException {
        ...
        if (usingCustomAST) {
            // when we are using a custom ast override Parser.getAST to return
            // the
            // custom AST type
            println("public new " + labeledElementASTType + " getAST()");
            println("{");
    ...
    public CSharpBlockFinishingInfo genCommonBlock(AlternativeBlock blk,
        ...
        if (nLL1 >= makeSwitchThreshold) {
            ...
            println("{");
            // tabs++;
            // update block nesting level as it affects creation of _saveIndex
            // OK?
            blockNestingLevel++;
        ...
        if (createdLL1Switch) {
            ...
            // return new CSharpBlockFinishingInfo(ps+"}",true,nIF>0); // close
            // up switch statement
                ...
                case GrammarElement.AUTO_GEN_NONE:
                    if (usingCustomAST
                            || ((el instanceof GrammarAtom) && (((GrammarAtom) el)
                                    .getASTNodeType() != null)))
                        println("astFactory.addASTChild(ref currentAST, (AST)"
                                + astName + ");");
                    else
                        println("astFactory.addASTChild(ref currentAST, "
                                + astName + ");");
                    break;
                case GrammarElement.AUTO_GEN_CARET:
                    if (usingCustomAST
                            || ((el instanceof GrammarAtom) && (((GrammarAtom) el)
                                    .getASTNodeType() != null)))
                        println("astFactory.makeASTRoot(ref currentAST, (AST)"
                                + astName + ");");
                    else
                        println("astFactory.makeASTRoot(ref currentAST, "
                                + astName + ");");
                    break;
                default:
                    break;
    ...
    protected void genTokenTypes(TokenManager tm) throws IOException {
        ...
        // class because
        // they are all constants and CSharp inteface types cannot contain
        // constants.
    ...
    protected void genTokenDefinitions(TokenManager tm) throws IOException {
        ...
        println("public const int NULL_TREE_LOOKAHEAD = "
    ...
    public String getASTCreateString(Vector v) {
        ...
        buf.append("(" + labeledElementASTType + ") astFactory.make(");
     ...
     * type -- ctorID.
     * 
     * IF the token type is a valid token symbol AND it has an associated AST
     * node type AND this is not a #[ID, "T", "ASTType"] constructor THEN
     * generate a call to factory.create(ID, Text, token.ASTNodeType())
     * 
     * #[ID, "T", "ASTType"] constructors are mapped to astFactory.create(ID,
     * "T", "ASTType")
     * 
     * The supported AST constructor forms are: #[ID] #[ID, "text"] #[ID,
     * "text", ASTclassname] -- introduced in 2.7.2
     * 
     * @param astCtorArgs
     *            The arguments to the AST constructor
     */
