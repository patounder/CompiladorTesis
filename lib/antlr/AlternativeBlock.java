 ...
 *
 * $Id: AlternativeBlock.java,v 1.1 2007-10-19 15:55:01 soto-r Exp $
 */
...
class AlternativeBlock extends AlternativeElement {
    ...
    protected int alti, altj; // which alts are being compared at the moment

    // with

    // deterministic()?
    protected int analysisAlt; // which alt are we computing look on? Must be

    // alti or altj

    protected boolean hasAnAction = false; // does any alt have an action?

    protected boolean hasASynPred = false; // does any alt have a syntactic
    ...
    boolean greedySet = false; // but, if not explicitly greedy, warning might

    // be generated

    protected boolean doAutoGen = true; // false if no AST (or text) to be

    // generated for block

    protected boolean warnWhenFollowAmbig = true; // warn when an empty path

    // or exit path

    protected boolean generateAmbigWarnings = true; // the general warning
    ...
    /**
     * Walk the syntactic predicate and, for a rule ref R, remove the ref from
     * the list of FOLLOW references for R (stored in the symbol table.
     */
        ...
        for (int i = 0; i < alternatives.size(); i++) {
            ...
            while (elem != null) {
                if (elem instanceof RuleRefElement) {
                    ...
                    if (rs == null) {
                        grammar.antlrTool.error("rule " + rr.targetRule
                                + " referenced in (...)=>, but not defined");
                    } else {
                        rs.references.removeElement(rr);
                    }
                } else if (elem instanceof AlternativeBlock) {// recurse into
                    // subrules
    ...
    public void setOption(Token key, Token value) {
        if (key.getText().equals("warnWhenFollowAmbig")) {
            ...
            if (value.getText().equals("true")) {
                ...
                grammar.antlrTool
                        .error(
                                ...
                                grammar.getFilename(), key.getLine(), key
                                        .getColumn());
            }
            ...
            if (value.getText().equals("true")) {
                ...
                grammar.antlrTool.error(
                        "Value for greedy must be true or false", grammar
                                .getFilename(), key.getLine(), key.getColumn());
            }
        } else {
            grammar.antlrTool.error("Invalid subrule option: " + key.getText(),
                    grammar.getFilename(), key.getLine(), key.getColumn());
        }
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < alternatives.size(); i++) {
            ...
            if (k == GrammarAnalyzer.LOOKAHEAD_DEPTH_INIT) {
                ...
                s += " {";
                for (int j = 1; j <= k; j++) {
                    s += cache[j].toString(",", grammar.tokenManager
                            .getVocabulary());
