...
public class LLkDebuggingParser extends LLkParser implements DebuggingParser {
    ...
    protected boolean fireSemanticPredicateEvaluated(int type, int num,
            boolean condition) {
        if (isDebugMode())
            return parserEventSupport.fireSemanticPredicateEvaluated(type, num,
                    condition, inputState.guessing);
        else
            return condition;
    }
    ...
    protected void fireSyntacticPredicateFailed() {
        if (isDebugMode())
            parserEventSupport
                    .fireSyntacticPredicateFailed(inputState.guessing);
    }

    protected void fireSyntacticPredicateStarted() {
        if (isDebugMode())
            parserEventSupport
                    .fireSyntacticPredicateStarted(inputState.guessing);
    }

    protected void fireSyntacticPredicateSucceeded() {
        if (isDebugMode())
            parserEventSupport
                    .fireSyntacticPredicateSucceeded(inputState.guessing);
    }
    ...
    public void match(int t) throws MismatchedTokenException,
        ...
        try {
            ...
            if (inputState.guessing == 0)
                parserEventSupport.fireMismatch(la_1, t, text,
                        inputState.guessing);
            throw e;
        }
    }
    ...
    public void match(BitSet b) throws MismatchedTokenException,
        ...
        try {
            ...
            if (inputState.guessing == 0)
                parserEventSupport.fireMismatch(la_1, b, text,
                        inputState.guessing);
            throw e;
        }
    }
    ...
    public void matchNot(int t) throws MismatchedTokenException,
        ...
        try {
            ...
            if (inputState.guessing == 0)
                parserEventSupport.fireMismatchNot(la_1, t, text,
                        inputState.guessing);
            throw e;
        }
    }
    ...
    protected void setupDebugging(TokenStream lexer, TokenBuffer tokenBuf) {
        ...
        try {
            try {
                ...
                System.err
                        .println("Swing is required to use ParseView, but is not present in your CLASSPATH");
                System.exit(1);
            }
            Class c = Utils.loadClass("antlr.parseview.ParseView");
            Constructor constructor = c.getConstructor(new Class[] {
                    LLkDebuggingParser.class, TokenStream.class,
                    TokenBuffer.class });
            constructor.newInstance(new Object[] { this, lexer, tokenBuf });
        } catch (Exception e) {
            System.err.println("Error initializing ParseView: " + e);
            System.err
                    .println("Please report this to Scott Stanchfield, thetick@magelang.com");
            System.exit(1);
        }
    }
